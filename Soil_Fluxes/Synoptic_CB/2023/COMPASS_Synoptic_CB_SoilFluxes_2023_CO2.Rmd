...
allflux <- allflux %>%
  ...
  slice_head(n = 1) %>%
  ungroup() %>%
  select (-c(base_plot, replicate, suffix))
...
fixedplots <- c("2-Jul-GWI-UP-3", "2-Aug-SWH-TR-6a", "1-Oct-SWH-TR-9", "1-Sep-SWH-TR-9",
                ...
                "1-May-SWH-UP-1", "1-May-SWH-UP-5", "1-Sep-MSM-TR-1", "2-Oct-GCW-TR-3", "2-Sep-SWH-TR-1", "2-Sep-SWH-TR-3", "2-Sep-SWH-UP-1")
...
# Create a new dataframe by removing rows negative CO2 fluxes
# allflux_filtered <- allflux[!(allflux$Plot %in% fluxes_weird$Plot), ] ##Already checked fluxes that were present in fluxes_weird which have a low R2 and a flux higher than 1 (23 obs) and removed the ones that are actually bad below. 

##Need to remove negative fluxes
allflux_filtered <- allflux[!(allflux$Plot %in% fluxes_negative$Plot), ]

#Any others that are not in the negative category that need to be removed: 
  #also removing ones that are crazy off/messy (did this by listing them below)
allflux_filtered <- allflux_filtered %>%
  filter(!Plot %in% c("1-Aug-SWH-TR-10a", "1-Sep-GCW-UP-3", "2-May-SWH-UP-1")) 

# 1-Aug-SWH-TR-10a - poor flux, need to remove
# 1-Sep-GCW-UP-3 - poor flux, need to remove
# 2-May-SWH-UP-1 - poor flux, need to remove
...
write.csv(allflux_filtered, file="Processed Data/COMPASS_Synoptic_CB_SmartChamber_Fluxes_Final_2023_CO2.csv", row.names = FALSE)           #Change file name
