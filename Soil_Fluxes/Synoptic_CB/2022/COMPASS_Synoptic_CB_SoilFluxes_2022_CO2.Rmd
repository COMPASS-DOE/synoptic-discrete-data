---
title: "COMPASS_Synoptic_SmartChamberFluxes_2022_CO2"
...
metadata <- metadata %>%
  ...
  mutate(Total_Time = 59)

##Write out to metadata with new total times 
# metadata <- metadata %>%
#   mutate(Total_Time = 59)
# write.csv(metadata, file="Raw Data/COMPASS_SmartChamber_Metadata_2022.csv") 
...
alldat_IDs <- alldat %>%
  ...
  mutate(Plot = if_else(str_detect(Plot, "UP8"), 
                        ...
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, ""), 
                        str_replace(Plot, "UP8", "UP_8"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "UP8"), 
                        str_replace(Plot, "UP8", "UP_8"), 
...
allflux_filtered <- allflux_filtered[,-c(32:33)]
