...
tobechecked <- c("2-Jul-SWH-TR-9d", "1-Jul-SWH-TR-9d", "1-May-GCW-TR-3a", "2-May-GCW-TR-3", "1-Aug-SWH-TR-10a", "1-Sep-GCW-UP-3")
...
fixedplots <- c("2-Jul-GWI-UP-3", "2-Aug-SWH-TR-6a", "1-Oct-SWH-TR-9", "1-Sep-SWH-TR-9",
                ...
                "2-May-SWH-TR-6a", "2-Sep-SWH-TR-1a", "1-Jul-SWH-TR-9d")
...
allflux <- allflux %>%
    mutate(qaqc_flag = ifelse(is.na(qaqc_flag) & Plot %in% neg_plots, "negative CO2 flux", qaqc_flag))
...
#List the ones that are V-shaped - assessed visually for now 
v_flags <- c(
"2-Jul-SWH-TR-10", 
"1-Jun-MSM-TR-3" , 
"1-Jun-MSM-TR-4" , 
...
allflux <- allflux %>%
    mutate(qaqc_flag = ifelse(is.na(qaqc_flag) & Plot %in% v_flags, "V-shaped, flux compromised", qaqc_flag))


########## W-shaped fluxes

#List the ones that are w-shaped - assessed visually for now 
w_flags <- c("1-Sep-SWH-TR-7a")

#add a flag for the fluxes that are v-shaped and therefore not even a flux 
allflux <- allflux %>%
    mutate(qaqc_flag = ifelse(is.na(qaqc_flag) & Plot %in% w_flags, "w-shaped, flux compromised", qaqc_flag))


########## Poor fluxes

#List the ones that are w-shaped - assessed visually for now 
poor_flags <- c("2-Jul-SWH-TR-9d", "1-Jul-SWH-TR-9d")
