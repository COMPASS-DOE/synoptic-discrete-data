...
tree_dat24 <- tree_dat %>%
  select(!"DBH_2022_cm")
dat24_wtree <- left_join(dat24, tree_dat24, by = c("Site", "Zone", "Tree_Code", "Replicate"))

dat24_wtree <- dat24_wtree %>%
...
#add sampling date and the month # to dat24
dat24_wtree_dates <- dat24_wtree %>%
  left_join(dates %>% select(Site, Year, Month, Day),
            by = c("Site", "Year", "Month")) %>%
  mutate(Month_num = match(Month, month.name))
...
all_dat <- rbind(dat23_wtree_dates, dat24_wtree_dates)
