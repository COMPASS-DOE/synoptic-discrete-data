...
final_dat1 <- final_dat %>% 
  mutate(Project = "COMPASS:Synoptic", 
         Region = "CB") %>% 
  rename(Day = Date) %>% 
  select(c("Project", "Region", "Year", "Month", "Day", "Time_24hr",  "Time_Zone", "Site", "Zone", "Topography", "Depth", "Avg_mV", "Std_dv", "Std_err", "Notes"))
...
metadat <- data.frame(Column_Names = colnames(final_dat1))
metadat
...
write.csv(final_dat1, "COMPASS_SynopticCB_Redox_AllData.csv", row.names = FALSE)


#write out a csv of the metadata associated with the data:
write.csv(metadat, "COMPASS_SynopticCB_Redox_Metadata.csv")
...
kable(
  summary_all_depths_years[, c("Site", "Zone_1", "min_mV",	"max_mV", "mean_mV")],  
  format = "latex",
  booktabs = TRUE,
