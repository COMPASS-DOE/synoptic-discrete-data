...
all_dat <- all_dat %>%
  ...
  mutate(Zone = case_when(Zone_1 == "Upland" ~ "UP", 
                          Zone_1 == "Transition" ~ "TR", 
                          Zone_1 == "Wetland" ~ "WC", 
                          Zone_1 == "Swamp" ~ "SWAMP", 
                          Zone_1 == "Transition-Hummock" ~ "TR-HUMMOCK", 
                          Zone_1 == "Transition-Hollow" ~ "TR-HOLLOW", 
                          TRUE ~ Zone)) 
all_dat <- all_dat %>%
  select (-Zone_1)
...
write.csv(all_dat, "COMPASS_SynopticCB_Redox_AllData.csv", row.names = FALSE)
...
data_plotting$Zone = factor(data_plotting$Zone, levels = c("UP", "SWAMP", "TR", "TR-HUMMOCK", "TR-HOLLOW", "WC"))
...
viz_redox_plot <- ggplot(data_plotting_all, aes(x = as.character(Year), y = Avg_mV, fill = Zone)) +
