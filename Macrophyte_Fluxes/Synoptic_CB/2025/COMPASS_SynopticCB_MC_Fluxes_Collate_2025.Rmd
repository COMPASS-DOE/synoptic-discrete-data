...
# Combine all files
all_dat <- merge(ch4, co2, by = c("Year", "Plot", "Site", "Zone", "Replicate", "Date", "TIMESTAMP", "TIMESTAMP_min", "TIMESTAMP_max", "Notes"))
...
##Read in notes for missing HydraGO data
hydrago_notes <- read.csv("Raw Data/HydraGO_NoData_Notes.csv")
hydrago_notes <- hydrago_notes %>% 
  select(Plot, HydraGO_Notes)

##Combine with flux data
all_dat1 <- all_dat %>% 
  left_join(hydrago_notes, by = "Plot")


##Read in hydrago data from S Drive
...
## Select only measurements from collars
hydrago <- hydrago %>% 
  filter(Type == "Collar")

## Remove columns that have replicates in flux data
hydrago <- hydrago %>% 
  select(-c("Memo", "Site", "Zone", "Type", "Replicate", "Month", "Month_Name", "Date", "Year", "Timestamp", "HydraGO.Serial", "HydraProbe.Serial"))


##Find fluxes that are missing hydrago data: 
missing_ids <- all_dat1 %>%
  filter(!(Plot %in% hydrago$Plot)) %>%
  filter(is.na(HydraGO_Notes)) %>%
  select(Plot) %>%
  rename(missing_ids = Plot)
...
##Separate IDs into columns
missing_ids <- missing_ids %>% 
  separate(
    col = missing_ids,
    sep = "_",
    into = c("Site", "Zone", "Collar" ,"Date"),
    remove = FALSE)
