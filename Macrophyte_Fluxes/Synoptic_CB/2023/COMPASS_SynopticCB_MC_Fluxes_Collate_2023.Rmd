...
ch4 <- read.csv("Processed Data/COMPASS_Synoptic_CB_Macrophyte_Fluxes_Final_2023_CH4.csv")
ch4$X <- NULL
ch4 <- ch4 %>%
  rename_with(
    .cols = 7:(ncol(.) - 4),
...
co2 <- read.csv("Processed Data/COMPASS_Synoptic_CB_Macrophyte_Fluxes_Final_2023_CO2.csv")
co2$X <- NULL
co2 <- co2 %>%
  rename_with(
    .cols = 7:(ncol(.) - 4),
...
# Combine all files
all_dat <- merge(ch4, co2, by = c("Year", "Plot", "Site", "Zone", "Replicate", "Date", "TIMESTAMP", "TIMESTAMP_min", "TIMESTAMP_max", "Notes"), all = FALSE)
...
hydrago <- read.csv("S:/Biogeochemistry/COMPASS Synoptics/Samples & Data/Soil Conductivity Probe/Raw Data Files/2023/COMPASS_Synoptic_2023_ALL_SoilCond_Data.csv")
...
write.csv(missing_ids, "fluxes_missing_hydrago_data_2023.csv", row.names = FALSE)
...
files <- files[
  ...
  grepl("2023", files, ignore.case = TRUE) &
...
#Unusually high CH4
high_ch4 <- all_dat %>% 
  filter(grepl("Unusually high CH4", ch4_qaqc_flag, ignore.case = TRUE))
perc_high_ch4 = nrow(high_ch4)/nrow(all_dat)*100

#Poor/messy flux
messy <- all_dat %>% 
  filter(grepl("messy", ch4_qaqc_flag, ignore.case = TRUE) | grepl("messy", co2_qaqc_flag, ignore.case = TRUE))
...
cat(paste0("Number unusually high CH4 fluxes removed: ", nrow(high_ch4)))
...
cat(paste0("Percentage unusually high CH4 fluxes removed: ", round(perc_high_ch4, digits = 2), "%"))
...
write.csv(final_dat, "COMPASS_SynopticCB_MC_Fluxes_2023.csv")
