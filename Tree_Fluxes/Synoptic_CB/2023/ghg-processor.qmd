...
#metadat$TimeOnly <- format(metadat$Time_start, "%H:%M:%S")
  
#dat_w_meta <- ffi_metadata_match(
  data_timestamps = dat$TIMESTAMP,
  start_dates = metadat$Date,
  start_times = metadat$TimeOnly,
...
fit_model <- function(df, depvar) {
  ...
  res[numerics] <- signif(res[numerics], 3)
  res
}
...
obs_list <- split(results, results$Site)
for (result in obs_list) {
  
  formatRound(datatable(result),
              which(sapply(result, is.numeric)), digits = 3)
  
  print(ggplot(result, aes(x = TIMESTAMP, y = flux_estimate, color = Plot)) + 
    geom_point() + 
...
plot_group <- function(results, combined_dat) {
  ...
  print(ggplot(plotdata_long, aes(ELAPSED_SECS, value, group = Obs, color = Gas)) + 
...
# Made a function above that prints each plot in order for it to iteratively print 
# in a loop, this is not always necessary, so this is the same function without the # print statement
plot_without_print <- function(results, combined_dat) {
