...
###### Run information - PLEASE CHANGE
  ...
  run_notes = "The second standard curve run had a very high end standard - concentration calculations were done with first standard curve.
                The instrument was having some issues during this run and some samples had no peaks as a result - these were removed in this code.
               Background concentrations were high on this run - we may need to consider subtracting the DI values. "  #any notes from the run
...
#file path and name for raw summary data file 
    raw_file_name = "Raw Data/COMPASS_SynopticCB_PW_DIC_202511.txt" 
    
#file path and name for raw all peaks file 
    raw_allpeaks_name = "Raw Data/COMPASS_SynopticCB_PW_DIC_202511_allpeaks.txt"

#file path and name of processed data file 
    processed_file_name = "Processed Data/COMPASS_SynopticCB_PW_Processed_DIC_202511.csv"
...
#need to remove the samples with no peaks: 
dat_raw <- dat_raw %>% 
  filter(!sample_name %in% c("202511_MSM_SW_C", "202511_GCW_SW_A", "202511_GCW_SW_B", 
                           "202511_GWI_TR_LysC_20cm", "202511_GWI_WC_LysB_10cm", 
                           "202511_SWH_SWAMP_LysC_10cm_WARM"))
...
blks_raw <- raw_file_name %>% 
  ...
  filter(run_datetime < "11/22/2025 10:05:38 PM") %>%
  filter(run_datetime > "11/20/2025 3:59:16 PM") %>%
  bind_rows() 

blank_avg <- mean(blks_raw$ic_raw)
...
crm_Chk_blank <- crm_raw %>%
  summarise(
    Mean_crm = mean((ic_raw-blank_avg), na.rm = TRUE),
    SD_crm = sd(ic_raw, na.rm = TRUE),
    CV = (SD_crm / Mean_crm) * 100,
    pct_diff_Chk = abs(Mean_crm - ic_crm) / ic_crm,
    crm_flag = ifelse(abs(pct_diff_Chk) < 25, "YES, Pass", "NO, rerun"))
...
#flag write out 
ifelse(crm_Chk_blank$pct_diff_Chk >= chk_flag, 
       "IC crm has a % Difference >25% of expected - REASSESS",  
       "IC crm has a % Difference <25% of expected - PROCEED")
cat("Run mean minus blank avg. =", crm_Chk_blank$Mean_crm, "\n")
cat("Expected  =", ic_crm, "\n\n")
...
#chks_raw$ic_raw <- chks_raw$ic_raw - blank_avg
...
## Assess Duplicates - No duplicates included on this run 
...
colnames(IDs) <- c("Date" , "Site_Code","Zone", "Lysimeter", "Depth", "Extra")
#head(IDs)
