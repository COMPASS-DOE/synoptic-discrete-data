...
###### Run information - PLEASE CHANGE
  Date_Run = "07/29/23"  #Date that instrument was run
...
#file path and name for raw summary data file 
    raw_file_name = "Raw Data/TOCTN_COMPASS_Synoptic_DIC_202307.txt" 
...
#file path and name of processed data file 
    processed_file_name = "Processed Data/COMPASS_SynopticCB_PW_Processed_DIC_202307.csv"
...
blks_raw <- raw_file_name %>% 
  ...
  filter(!str_detect(sample_name, "KPM")) %>%
  filter(!str_detect(sample_name, "DK")) %>%
  bind_rows() 
...
dups$sample_name<-gsub("dup","",as.character(dups$sample_name))
