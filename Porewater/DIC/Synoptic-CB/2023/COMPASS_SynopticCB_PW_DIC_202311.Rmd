...
###### Run information - PLEASE CHANGE
  Date_Run = "11/10/23"  #Date that instrument was run
...
#file path and name for raw summary data file 
    raw_file_name = "Raw Data/TOCTN_COMPASS_Synoptic_DIC_202311.txt" 
...
#file path and name of processed data file 
    processed_file_name = "Processed Data/COMPASS_SynopticCB_PW_Processed_DIC_202311.csv"
...
#Looks like one sample was run twice and we should remove it & one duplicate was not id'd as a dup:
#dat_raw <- dat_raw %>%
  #filter(!(sample_name == "GWI_202306_SW_A" & ic_raw == 9.863)) %>%
#  mutate(sample_name = str_replace(sample_name, "LYs", "Lys"))   %>%
#  mutate(
 #   sample_name = case_when(
#      sample_name == "202310_MSM_SW_B" & ic_raw == "20.04" ~ paste0(sample_name, "_dup"),
#      TRUE ~ sample_name
#    )
#  )
...
## Assessing Standard Curves - the curve was assessed manually on the instrument
