...
###### Run information - PLEASE CHANGE
  Date_Run = "10/04/2024"  #Date that instrument was run
  Run_by = "Stephanie J. Wilson"  #Instrument user 
  Script_run_by = "Stephanie J. Wilson" #Code user 
  run_notes = "There is one sample: 202409_GWI_TR_LysA_45cm that is not in the metadata and 
    There were no other samples collected from that lysimeter, so it cannot be that sample. 
    There are two other lysimeters that could have yielded this sample, but no way to tell so 
    we will remove this sample from the analysis - this is done in this code."  #any notes from the run
...
#file path and name for raw summary data file 
    raw_file_name = "Raw Data/TOCTN_COMPASS_Synoptic_DIC_202409.txt" 
    
#file path and name for raw all peaks file 
    raw_allpeaks_name = "Raw Data/TOCTN_COMPASS_Synoptic_DIC_202409_allpeaks.txt"

#file path and name of processed data file 
    processed_file_name = "Processed Data/COMPASS_SynopticCB_PW_Processed_DIC_202409.csv"
...
dat_raw <- dat_raw %>% 
  filter(!sample_name == "202409_GWI_TR_LysA_45cm") #remove sample not in the metadata
