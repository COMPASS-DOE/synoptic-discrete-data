...
data <- read.csv("QAQC'd Data/COMPASS_H2S_202408_plate1-3_Summary_Data.csv")
reruns <- read.csv("QAQC'd Data/COMPASS&TidalSurveyReruns-Inc_H2S_202408_Summary_Data.csv")
...
#Read in Sample IDs
SampleIDs <- read_excel("202408_SampleList.xlsx")
...
SampleIDs <- SampleIDs %>%
  separate(
    col = "Sample ID",
    sep = "_",
    into = c("Date" ,"Site", "Zone", "Replicate", "Depth"),
    remove = FALSE)

SampleIDs$`Sample ID` <- NULL
```
...
final_data <- merged_data %>% 
  mutate(
    ...
    Region = "CB", 
    Analysis_rundate = "20240903" #need to make it so this comes out in plate processed file
  ) 
...
write.csv(final_data, "COMPASS_H2S_202408_Plates_Combined_Data.csv")

#Write out samples that still have high CVs
write.csv(HighCV, "COMPASS_H2S_202408_Plates_Combined_HighCVs.csv")

#Write out ADL samples
write.csv(ADL_not_fixed, "COMPASS_H2S_202408_Plates_Combined_ADL.csv")

#Write out BDL samples
write.csv(BDL_not_fixed, "COMPASS_H2S_202408_Plates_Combined_BDL.csv")
