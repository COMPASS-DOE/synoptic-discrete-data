...
Run_notes="Plate 2 std curve used. 
MC's both bad on Plate 3, MC 10 bad on Plate 1.  
Plate 1 had a bad dup and spk. 
Some samples were still adl after dilution. 
"#any notes from run

#Stds that should be excluded
stds_to_remove<-data.frame(Plate=c("Plate1", "Plate3"),IDs=c("Std 3", "Std 1"))
# stds_to_remove<-NA
...
file_paths_rawdata <- list.files(path = "Raw Data", pattern = yearmonth, full.names = FALSE)

file_paths_tidydata <- list.files(path = "Tidy Data", pattern = yearmonth, full.names = FALSE)
head(file_paths_tidydata)

#file path name for processed data
#Write out final data frame 
final_data_path <- "Processed Data/COMPASS_H2S_202404_Plates_Combined.csv"
HighCV_path <- "Processed Data/COMPASS_H2S_202404_Plates_HighCVs.csv"
ADL_not_fixed_path <- "Processed Data/COMPASS_H2S_202404_Plates_ADL.csv"
BDL_not_fixed_path <- "Processed Data/COMPASS_H2S_202404_Plates_BDL.csv"
...
# make a new dataframe for the high cv points for each plate
  ...
  std_plate1$H2S_cv_flag <- as.character(std_plate1$H2S_cv_flag)
  ...
  H2S_stds1_all_fixed$H2S_cv_flag <- as.character(H2S_stds1_all_fixed$H2S_cv_flag)
  H2S_stds1_all$H2S_cv_flag <- as.character(H2S_stds1_all$H2S_cv_flag)
...
H2S <- ggplot(H2S_stds, aes(Conc, Abs)) + geom_point() + geom_smooth(method = "lm", se = FALSE)+ 
  ...
  stat_regline_equation(label.x = 15, label.y = 1.6) + # Add the equation
  stat_regline_equation(aes(label = ..adj.rr.label..), label.x = 15, label.y = 1.3) # Add R squared
H2S
