...
author: "September 2024 All Plates"
date: "`r Sys.Date()`"
output: 
  pdf_document:
    toc: false
...
##Run Notes
```{r}
run_notes = "Some sample IDs are missing from metadata: GWI_202410_UP_LYSA_20CM, GWI_202410_TR_LYSC_45CM"
```
...
data <- read.csv("QAQC'd Data/COMPASS_H2S_202409_plate1-8_Summary_Data.csv")
reruns <- read.csv("QAQC'd Data/COMPASS_H2S_20241007_Reruns_Plate1_Summary_Data.csv")

all_data <- rbind(data, reruns)
all_data$X <- NULL
...
Sample_data_nodups <- Sample_data_wr %>%
  arrange(IDs, H2S_cv) %>% # Sort by ID ascending, CV ascending
...
final_data <- merged_data %>% 
  mutate(
    ...
    Region = "CB", 
    Analysis_rundate = "20241007", #need to make it so this comes out in plate processed file
    Run_notes  = run_notes
  ) 
...
write.csv(final_data, "COMPASS_H2S_202409_Plates_Combined_Data.csv")

#Write out samples that still have high CVs
write.csv(HighCV, "COMPASS_H2S_202409_Plates_Combined_HighCVs.csv")

#Write out ADL samples
write.csv(ADL_not_fixed, "COMPASS_H2S_202409_Plates_Combined_ADL.csv")

#Write out BDL samples
write.csv(BDL_not_fixed, "COMPASS_H2S_202409_Plates_Combined_BDL.csv")
