...
all_data <- read.csv("QAQC'd Data/COMPASS_H2S_202405_plate1-8_Summary_Data.csv")
all_data$X <- NULL
...
Sample_data <- all_data %>% 
  ...
  mutate(IDs = gsub(" x25", "", as.character(IDs)))
...
#Read in Sample IDs
SampleIDs <- read_excel("202405_SampleList.xlsx")
...
SampleIDs <- SampleIDs %>%
  mutate(Sample_ID_old = paste(Site, Date, Zone, Replicate, Depth, sep = "_"))

string_to_find <- "MSM_202405_UP_LysA_10cm"
found <- any(sapply(SampleIDs, function(x) any(grepl(string_to_find, x))))
...
base_cond <- with(SampleIDs,
  Site == "MSM" & Date == "202405" & Zone == "UP")

if (any(base_cond & SampleIDs$Replicate == "LysA" & SampleIDs$Depth == "10cm")) {

  SampleIDs <- SampleIDs %>%
    mutate(
      Replicate = case_when(
        base_cond & Replicate == "LysA" ~ "LysB",
        base_cond & Replicate == "LysB" ~ "LysC",
        base_cond & Replicate == "LysC" ~ "LysA",
        TRUE ~ Replicate
      )
    )
}

SampleIDs$Sample_ID_old <- NULL
...
sulfide_metadata_selected <- sulfide_metadata %>%
  dplyr::select(H2S_ID, Year, Month, Day, Time..24hr., Time.Zone_EDT.EST,  Field.Notes) 
...
final_data <- merged_data %>% 
  mutate(
    ...
    Region = "CB", 
    Analysis_rundate = "20240529", #need to make it so this comes out in plate processed file
    Run_notes = run_notes 
  ) 
...
final_data <- final_data %>%
  ...
  dplyr::select(
    Project, Region, Site, Zone, Replicate, Depth_cm, 
         ...
         H2S_mean, H2S_sd, H2S_cv, H2S_Conc_flag, H2S_cv_flag, Dilution, 
...
write.csv(final_data, "COMPASS_H2S_202405_Plates_Combined_Data.csv")

#Write out samples that still have high CVs
write.csv(HighCV, "COMPASS_H2S_202405_Plates_Combined_HighCVs.csv")

#Write out ADL samples
write.csv(ADL_not_fixed, "COMPASS_H2S_202405_Plates_Combined_ADL.csv")

#Write out BDL samples
write.csv(BDL_not_fixed, "COMPASS_H2S_202405_Plates_Combined_BDL.csv")
