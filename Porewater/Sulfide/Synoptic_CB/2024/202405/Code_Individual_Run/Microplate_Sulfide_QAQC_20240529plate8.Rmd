...
dat <- stds <- read_plate(
  file = "S:/GlobalChangeEco/porewater_r scripts & Data/Data/Sulfide Microplate/COMPASS/2024 May/Tidy data/20240529_COMPASS_H2S_plate8.csv",             # full path to the .csv file
...
if (qlog_Date$Slope > (mean(qlog$Slope)+ (2*sd(qlog$Slope)))| qlog_Date$Slope < (mean(qlog$Slope)- (2*sd(qlog$Slope)))) {
print("rerun Plate")
} else {
print("continue")
}
...
int1 <- ggplot(data=qlog, aes(x=Date, y=Intercept)) +
  ...
  geom_hline(yintercept= (mean(qlog$Intercept)- (2*sd(qlog$Intercept))), linetype="dashed", color = "red", size=2)+
  geom_point(aes(size=3)) + 
  theme_classic() + ylim(0.0,0.1) + 
  theme(legend.position="none")+ 
  ggtitle("Sulfide Intercepts")

int1

if (qlog_Date$Intercept > (mean(qlog$Intercept)+ (2*sd(qlog$Intercept)))| qlog_Date$Intercept < (mean(qlog$Intercept)- (2*sd(qlog$Intercept)))) {
print("rerun Plate")
} else {
print("continue")
}
...
head(dat_IDs)
write.csv(dat_IDs, file="S:/GlobalChangeEco/Porewater_R scripts & Data/Data/Sulfide Microplate/COMPASS/2024 May/QAQC'd Data/COMPASS_H2S_20240529_plate8_Summary_Data.csv")

#Read out all the data in dat 
head(dat)
write.csv(dat, file="S:/GlobalChangeEco/Porewater_R scripts & Data/Data/Sulfide Microplate/COMPASS/2024 May/QAQC'd Data/COMPASS_H2S_20240529_plate8_Data.csv")
...
dat3 <- dat3[ ,-(2:5)]
head(dat3)
write.csv(dat3, file="S:/GlobalChangeEco/Porewater_R scripts & Data/Data/Sulfide Microplate/COMPASS/2024 May/QAQC'd Data/COMPASS_H2S_20240529_plate8_short_Data.csv")
