...
Run_notes="Plate 3 MC 10 and 20 were bad. 
1/7 plates had a bad dup. 
4/7 plates had a bad spike. 
"#any notes from run
...
Old_ID_1 = "SHW_UPCON_LysB_45cm"
New_ID_1 = "SWH_UPCON_LysB_45cm"
...
file_paths_tidydata <- list.files(path = "Tidy Data", pattern = Month, full.names = TRUE)
head(file_paths_tidydata)

#file path name for processed data
#Write out final data frame 
final_data_path <- "Processed Data/COMPASS_H2S_202305_Plates_Combined.csv"
HighCV_path <- "Processed Data/COMPASS_H2S_202305_Plates_HighCVs.csv"
ADL_not_fixed_path <- "Processed Data/COMPASS_H2S_202305_Plates_ADL.csv"
BDL_not_fixed_path <- "Processed Data/COMPASS_H2S_202305_Plates_BDL.csv"

#QAQC log path
log_path="Raw Data/Sulfide_STD_QAQC.csv"

#Sample ID Path
SampleIDs1=paste0("Raw Data/COMPASS_SampleIDs_202305.xlsx")
SampleIDs2=paste0("Raw Data/COMPASS_SampleIDs_202305.xlsx")
...
#Note from Steph: Check all sample IDs from MSM Upland in 2023 May samples. Samples were labeled LysA, but are actually LysC from this month and zone only and need documented. If there are samples from MSM UP LysA 10cm listed, then the samples were NOT corrected and that would mean that all of the LysA samples should be LysC and we should document and correct. We know this because we hardly ever get water from LysA 10cm at MSM. This is a one time issue from May 2023 as far as I am aware. 
```{r Check and fix MSM Up Lys sample IDs, echo = FALSE}

string_to_find <- "MSM_UP_LysA_10cm"
found <- any(sapply(dat, function(x) any(grepl(string_to_find, x))))
