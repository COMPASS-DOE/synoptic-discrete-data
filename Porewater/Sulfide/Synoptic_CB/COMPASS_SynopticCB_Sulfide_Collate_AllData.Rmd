...
```{r Visualize Data, echo=FALSE, warning=FALSE, message = FALSE, fig.width=16, fig.height=10, dpi = 400}
...
```{r Summarized data for Site and Zone, echo=FALSE, warning=FALSE, message = FALSE, fig.width=16, fig.height=10, dpi = 300}
...
#Plot data and change colors based on Zone:
viz_H2S_depth <-  ggplot(data_plotting_depth, aes(x = Depth_cm, y = H2S_mean_uM, fill = Zone)) +
  geom_boxplot() +
  ...
  scale_fill_manual(values = c(
    ...
    "WC" = "#419973")) +
  theme_classic() +
  labs(x = "Sampling Depth (cm)", y = "Sulfide (uM)", title = "") +
  # theme(
  #   panel.spacing = unit(.05, "lines"), 
  #   panel.border = element_rect(color = "black", fill = NA, linewidth = 1))+ 
  theme(text = element_text(size = 30))+ 
...
## Boxplot summary
```{r Overall Boxplot, echo=FALSE, warning=FALSE, message = FALSE, fig.width=16, fig.height=10, dpi = 400}
...
# Save as a PNG
png("COMPASS_SynopticCB_H2S_alldata.png", width = 2000, height = 1200, res = 100) 
...
##There might be an issue here because the H2S_mean value already has a sd from the triplicates (or duplicates) on the plate. 
...
summary_by_year <- all_dat %>%
  group_by(Year, Site, Zone, Depth_cm) %>%
   reframe(
     min_H2S_mM = min(H2S_mean_uM, na.rm=TRUE),
     max_H2S_mM = max(H2S_mean_uM, na.rm=TRUE), 
     mean_H2S_mM = mean(H2S_mean_uM, na.rm=TRUE), 
     sd_H2S_mM = sd(H2S_mean_uM, na.rm=TRUE))
...
kable(
  summary_all_depths_years[, c("Site", "Zone", "min_H2S_mM",	"max_H2S_mM",	"mean_H2S_mM", "sd_H2S_mM")],  
  format = "latex",
  booktabs = TRUE,
...
#write out a csv of all the data to the main folder: 
write.csv(summary_by_year, "COMPASS_SynopticCB_PW_Sulfide_Summarized_By_Year.csv")


#write out a csv of the metadata associated with the data: 
write.csv(summary_all_years, "COMPASS_SynopticCB_PW_Sulfide_Summarized_Years_Combined.csv")


#write out a csv of the metadata associated with the data: 
write.csv(summary_all_depths_years, "COMPASS_SynopticCB_PW_Sulfide_Summarized_Years_and_Depths_Combined.csv")
```
