...
author: "May 2025 Samples Plate 2"
date: "`r Sys.Date()`"
output: 
  pdf_document:
    toc: false
...
###### Run information - PLEASE CHANGE
  ...
  Project = "COMPASS"
  run_notes = " Chk std 4 was out of range
...
#file path and name for raw summary data file 
  ...
  Sheet = "Plate 2"
  raw_file_name_data = "Raw Data/20250529_COMPASS_H2S_Plate2.xlsx"

#file path and name of processed data file 
  tidy_file_name = "Tidy Data/20250529_COMPASS_H2S_Plate2_tidy.csv" 
  processed_file_name = "Processed Data/COMPASS_H2S_20250529_Plate2.csv" 
  samples_need_dilution_name = "Processed Data/COMPASS_H2S_20250529_Plate2_SamplesNeedDilution.csv" 
  samples_high_cv_name = "Processed Data/COMPASS_H2S_20250529_Plate2_SamplesHighCV.csv" 
...
#Column 1
Column1= c("A01", "B01","C01", "D01", "E01", "F01")

#Column 2
Column2= c("A02", "B02","C02", "D02", "E02", "F02")

#Column 3
Column3= c("A03", "B03","C03", "D03", "E03", "F03")


#Delete Column one
stds_HCV1 <- subset(stds_HCV, !(Wells %in% Column1 ))

#Delete Column two
stds_HCV2 <- subset(stds_HCV, !(Wells %in% Column2 ))

#Delete Column three
stds_HCV3 <- subset(stds_HCV, !(Wells %in% Column3 ))
...
stds_HCV1 <- stds_HCV1 %>%
  ...
  summarise(H2S_mean_Abs = mean(Abs), H2S_sd = sd(Abs), H2S_cv = cv(Abs), 
            Dilution = first(Dilution), 
            Conc = first(Conc))
stds_HCV1$H2S_cv_flag <- ifelse(stds_HCV1$H2S_cv > cv_flag_sample, 'High CV', 'Within range')
...
stds_HCV2 <- stds_HCV2 %>%
  ...
  summarise(H2S_mean_Abs = mean(Abs), H2S_sd = sd(Abs), H2S_cv = cv(Abs), 
            Dilution = first(Dilution), 
            Conc = first(Conc))
stds_HCV2$H2S_cv_flag <- ifelse(stds_HCV2$H2S_cv > cv_flag_sample, 'High CV', 'Within range')
...
stds_HCV3 <- stds_HCV3 %>%
  ...
  summarise(H2S_mean_Abs = mean(Abs), H2S_sd = sd(Abs), H2S_cv = cv(Abs), 
            Dilution = first(Dilution), 
            Conc = first(Conc))
stds_HCV3$H2S_cv_flag <- ifelse(stds_HCV3$H2S_cv > cv_flag_sample, 'High CV', 'Within range')


#find lowest CVs
stds_HCV1_1 <- subset(stds_HCV1, stds_HCV1$H2S_cv < stds_HCV2$H2S_cv & stds_HCV1$H2S_cv < stds_HCV3$H2S_cv)
# head(stds_HCV1_1)

stds_HCV2_2 <- subset(stds_HCV2, stds_HCV2$H2S_cv < stds_HCV1$H2S_cv & stds_HCV2$H2S_cv < stds_HCV3$H2S_cv)
# head(stds_HCV1_1)

stds_HCV3_3 <- subset(stds_HCV3, stds_HCV3$H2S_cv < stds_HCV2$H2S_cv & stds_HCV3$H2S_cv < stds_HCV1$H2S_cv)
# head(stds_HCV3_3)


#recombine data frames
stds2_HCV <- rbind(stds_HCV1_1,stds_HCV2_2,stds_HCV3_3)
knitr::kable(stds2_HCV, caption = "Samples that originally had high CVs")

H2S_stds1 <- subset(H2S_stds1, (!IDs %in% stds2_HCV$IDs ))
H2S_stds1 <- rbind(stds2_HCV, H2S_stds1)
head(H2S_stds1)


## Calculate std curve
H2S_lm <- lm(H2S_stds1$Abs ~ H2S_stds1$Conc)
# summary(H2S_lm)
cf <- coef(H2S_lm)
...
H2S <- ggplot(H2S_stds1, aes(Conc, H2S_mean_Abs)) + 
  geom_point() + 
...
```{r Assess Check Standards, echo=FALSE, warning = FALSE, include=FALSE}
...
#Flag high cvs
H2S_std_Chk1$H2S_cv_flag <- ifelse(H2S_std_Chk1$H2S_cv > cv_flag_stds, 'High CV', 'Within range')

# filter High CV Samples
Chkstds_HCV <- H2S_std_Chk1 %>%  
  filter(str_detect(H2S_cv_flag, "High CV")) 
...
#Column 1
Column1= c("B07", "D07", "F10", "H10")

#Column 2
Column2= c("B08", "D08", "F11", "H11")

#Column 3
Column3= c("B09", "D09", "F12", "H12")


#Delete Column one
Chkstds_HCV1 <- subset(Chkstds_HCV, !(Wells %in% Column1 ))

#Delete Column two
Chkstds_HCV2 <- subset(Chkstds_HCV, !(Wells %in% Column2 ))

#Delete Column three
Chkstds_HCV3 <- subset(Chkstds_HCV, !(Wells %in% Column3 ))


##Find CV for each data set with one column's data removed
#W/out column1
Chkstds_HCV1 <- Chkstds_HCV1 %>%
  group_by(IDs) %>%
  summarise(H2S_mean_Abs = mean(Abs), H2S_sd = sd(Abs), H2S_cv = cv(Abs), 
            Dilution = first(Dilution), 
            Conc = first(Conc))
Chkstds_HCV1$H2S_cv_flag <- ifelse(Chkstds_HCV1$H2S_cv > cv_flag_sample, 'High CV', 'Within range')

#W/out column2
Chkstds_HCV2 <- Chkstds_HCV2 %>%
  group_by(IDs) %>%
  summarise(H2S_mean_Abs = mean(Abs), H2S_sd = sd(Abs), H2S_cv = cv(Abs), 
            Dilution = first(Dilution), 
            Conc = first(Conc))
Chkstds_HCV2$H2S_cv_flag <- ifelse(Chkstds_HCV2$H2S_cv > cv_flag_sample, 'High CV', 'Within range')

#W/out column3
Chkstds_HCV3 <- Chkstds_HCV3 %>%
  group_by(IDs) %>%
  summarise(H2S_mean_Abs = mean(Abs), H2S_sd = sd(Abs), H2S_cv = cv(Abs), 
            Dilution = first(Dilution), 
            Conc = first(Conc))
Chkstds_HCV3$H2S_cv_flag <- ifelse(Chkstds_HCV3$H2S_cv > cv_flag_sample, 'High CV', 'Within range')


#find lowest CVs
Chkstds_HCV1_1 <- subset(Chkstds_HCV1, Chkstds_HCV1$H2S_cv < Chkstds_HCV2$H2S_cv & Chkstds_HCV1$H2S_cv < Chkstds_HCV3$H2S_cv)
# head(Chkstds_HCV1_1)

Chkstds_HCV2_2 <- subset(Chkstds_HCV2, Chkstds_HCV2$H2S_cv < Chkstds_HCV1$H2S_cv & Chkstds_HCV2$H2S_cv < Chkstds_HCV3$H2S_cv)
# head(Chkstds_HCV1_1)

Chkstds_HCV3_3 <- subset(Chkstds_HCV3, Chkstds_HCV3$H2S_cv < Chkstds_HCV2$H2S_cv & Chkstds_HCV3$H2S_cv < Chkstds_HCV1$H2S_cv)
# head(Chkstds_HCV3_3)


#recombine data frames
Chkstds2_HCV <- rbind(Chkstds_HCV1_1,Chkstds_HCV2_2,Chkstds_HCV3_3)
knitr::kable(Chkstds2_HCV, caption = "Samples that originally had high CVs")

H2S_std_Chk1 <- subset(H2S_std_Chk1, (!IDs %in% stds2_HCV$IDs ))
H2S_std_Chk1 <- rbind(stds2_HCV, H2S_std_Chk1)
head(H2S_stds1)
