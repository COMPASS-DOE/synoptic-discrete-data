...
##Make Rmd look better
##Edit so doesn't just say "continue" but what it's checking for 
##Add column to add note if the QAQC fails and add run notes
...
## Plot standards 
```{r Plot Standards, echo=FALSE, message=FALSE, warning = FALSE, fig.height = 4}
...
Rsq1 <- ggplot(data=qlog, aes(x=Date, y=R2)) +
  geom_hline(yintercept= (r2_cutoff), linetype="dashed", color = "red", size=2)+
  geom_point(aes(size=3)) + 
...
####Chk stds were entered incorrectly - fix this####
H2S_std_Chk$IDs[H2S_std_Chk$IDs == "ChkStd 1"] <- "ChkStd 0"
H2S_std_Chk$IDs[H2S_std_Chk$IDs == "ChkStd 2"] <- "ChkStd 0"
H2S_std_Chk$IDs[H2S_std_Chk$IDs == "ChkStd 5"] <- "ChkStd 4"
H2S_std_Chk$IDs[H2S_std_Chk$IDs == "ChkStd 6"] <- "ChkStd 4"
...
## Flag sample data
```{r Flag sample data, echo=FALSE, warning = FALSE}
...
dat_flagged$H2S_info <- ifelse(dat_flagged$Conc <0, 'bdl', ifelse(dat_flagged$Abs > mean(std5$Abs), "adl", "Within_Range"))
...
## Calculate avg, std dev, and cv across wells
```{r}
head(dat_flagged)

####Data entered incorrectly -- fix here####
dat_flagged$IDs[dat_flagged$IDs == "17 Dup"] <- "16 Dup"
dat_flagged$IDs[dat_flagged$IDs == "18 Dup"] <- "16 Dup"
