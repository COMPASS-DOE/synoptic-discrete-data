...
Run_notes="Plate 1 had low MC 20 and Plate 4 had low MC 10. Plate 1 and 4 dups were bad. 
Majority of spikes were bad. Plate 3 Std curve used for all plates."#any notes from run
...
final_data_path <- "COMPASS_H2S_202511_Plates_Combined.csv"
HighCV_path <- "COMPASS_H2S_202511_Plates_HighCVs.csv"
ADL_not_fixed_path <- "COMPASS_H2S_202511_Plates_ADL.csv"
BDL_not_fixed_path <- "COMPASS_H2S_202511_Plates_BDL.csv"
...
```{r Format data 20251110, include=FALSE}
...
```{r Read in and Fix Sample IDs 20251110, include = FALSE}
...
```{r Format data 20251114, include=FALSE}
...
Sample_data2 <- all_data2 %>% 
  ...
  subset((!IDs %like% "Spike"))%>% 
  subset((!IDs %like% "TMP"))%>% 
  subset((!Plate %like% "Plate1"))%>% 
  subset((!Plate %like% "Plate2"))%>% 
  subset((!Plate %like% "Plate3"))

```

```{r Read in and Fix Sample IDs 20251114, include = FALSE}

#Read in Sample IDs
SampleIDs2 <- read_excel(SampleIDs2)
SampleIDs2$Number <- as.character(SampleIDs2$Number)
names(SampleIDs2) <- tools::toTitleCase(names(SampleIDs2))

```

```{r Convert Sample Numbers to Sample IDs 20251114, include = FALSE}
##Merge with data
...
```{r combine samples from 20251114 and 20251110, include=FALSE}
