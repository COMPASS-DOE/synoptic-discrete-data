...
Run_notes="September reruns plate was not run. 
Plate1 std curve used. 
Half of MCs were bad. 
Some samples still high CV and adl. 
7/11 spks were bad. 
Sample still had high CV and some samples adl. 
"#any notes from run
...
# New_ID_1 <- "2_Gcrew 202209_UP_LysB_10cm"
# 
# Old_ID_2 <- "4_Gcrew_ 202209_UP__LysB_45cm"
# New_ID_2 <- "4_Gcrew_202209_UP_LysB_45cm"
...
file_paths_tidydata <- list.files(path = "Raw Data", pattern = "202209_Plate", full.names = TRUE)
head(file_paths_tidydata)

#file path name for processed data
#Write out final data frame 
final_data_path <- "Processed Data/COMPASS_H2S_202209_Plates_Combined.csv"
HighCV_path <- "Processed Data/COMPASS_H2S_202209_Plates_HighCVs.csv"
ADL_not_fixed_path <- "Processed Data/COMPASS_H2S_202209_Plates_ADL.csv"
BDL_not_fixed_path <- "Processed Data/COMPASS_H2S_202209_Plates_BDL.csv"

#QAQC log path
log_path="Raw Data/Sulfide_STD_QAQC.csv"

#Sample ID Path
SampleIDs1=paste0("Raw Data/COMPASS_Synoptic_CB_H2S_202209_Lys_IDs.csv")
...
Sample_data_IDs1$Depth <-  ifelse((Sample_data_IDs1$'Sample.Name' %like% "SW"), "0", Sample_data_IDs1$Depth)
...
Sample_data_IDs1 <- Sample_data_IDs1 %>%
  mutate(Sample_ID = paste(Site, Date_Run, Zone, Rep, Depth_cm, sep = "_"))
...
Sample_data_IDs1 <- Sample_data_IDs1 %>%
  mutate(
    Sample_ID = if_else(
      ...
      # Modify the string:
      Sample_ID %>%
        str_replace("_A", "_LysA") %>%                    # Replace "_LysA" with "_A"
        str_replace("_B", "_LysB") %>%                    # Replace "_LysB" with "_B"
        str_replace("_C", "_LysC") ,                      # Remove trailing "_0cm"
      Sample_ID %>%                    
        str_replace("_0cm$", "")
    )
  )
...
Sample_data_nodups <- Sample_data_nodups %>%
  mutate(Rep = gsub("Lys", "", as.character(Rep)))%>%
  mutate(Rep = gsub("lys", "", as.character(Rep)))
...
all_data_selected <- Sample_data_nodups %>%
  dplyr::select(-c("ID_conc", "Depth", "Year", "Month"))
...
merged_data <- all_data_selected %>%
  left_join(sulfide_metadata_selected, by = c("Sample_ID" = "H2S_ID", "Rep" = "Lysimeter", "Depth_cm"))
