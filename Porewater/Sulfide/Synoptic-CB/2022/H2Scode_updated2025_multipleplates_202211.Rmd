...
Run_notes="Used Plate 4 standard curve. 9/15 of the MC's were off. 
A lot of the spks were bad but most of the samples that were spiked were 0 to begin with, 
so probably wouldn't have given a good spike. 
"#any notes from run
...
# Old_ID_1 <- "2_Gcrew 20208_UP_LysB_10cm"
# New_ID_1 <- "2_Gcrew 202211_UP_LysB_10cm"
# 
# Old_ID_2 <- "4_Gcrew_ 202211_UP__LysB_45cm"
# New_ID_2 <- "4_Gcrew_202211_UP_LysB_45cm"
...
file_paths_tidydata <- list.files(path = "Raw Data", pattern = "202211_Plate", full.names = TRUE)
head(file_paths_tidydata)

#file path name for processed data
#Write out final data frame 
final_data_path <- "Processed Data/COMPASS_H2S_202211_Plates_Combined.csv"
HighCV_path <- "Processed Data/COMPASS_H2S_202211_Plates_HighCVs.csv"
ADL_not_fixed_path <- "Processed Data/COMPASS_H2S_202211_Plates_ADL.csv"
BDL_not_fixed_path <- "Processed Data/COMPASS_H2S_202211_Plates_BDL.csv"

#QAQC log path
log_path="Raw Data/Sulfide_STD_QAQC.csv"

#Sample ID Path
SampleIDs1=paste0("Raw Data/COMPASS_Synoptic_CB_H2S_202211_Lys_IDs.csv")
...
H2S <- ggplot(H2S_stds, aes(Conc, Abs)) + geom_point() + geom_smooth(method = "lm", se = FALSE)+ 
  ...
  stat_regline_equation(label.x = 15, label.y = 1.5) + # Add the equation
...
dat1$ID_full <- toupper(dat1$ID_full)
spks <- dat1 %>%  filter(str_detect(ID_full, regex("spk", ignore_case = TRUE))) 
...
#put it back together with the old data set and look for duplicates 
dat2$ID_full <- toupper(dat2$ID_full)
...
Sample_data1 <- all_data1 %>% 
  ...
  subset((!IDs %like% "spk"))%>% 
  subset((!IDs %like% "TMP")) #%>% 
  # subset((!Plate %like% "Plate4"))%>% ##Why were these removed? 
  # subset((!Plate %like% "Plate5"))%>% 
  # subset((!Plate %like% "Plate6"))
...
#Read in Sample IDs
SampleIDs1 <- read.csv(SampleIDs1)
SampleIDs1$SO4_vial_no <- as.character(SampleIDs1$SO4_vial_no)
names(SampleIDs1) <- tools::toTitleCase(names(SampleIDs1))

```


```{r Convert Sample Numbers to Sample IDs 20251110, include = FALSE}
##Merge with data
Sample_data_IDs1 <- Sample_data1 %>%
  left_join(SampleIDs1, by = c("IDs" = "SO4_vial_no"))

##Make full sample IDs
##Depths for SW samples should be 0 
Sample_data_IDs1$Depth <-  ifelse((Sample_data_IDs1$'Sample.Name' %like% "SW"), "0cm", Sample_data_IDs1$Depth)
Sample_data_IDs1$Zone <-  ifelse((Sample_data_IDs1$'Sample.Name' %like% "SW"), "SW", Sample_data_IDs1$Zone)
# Sample_data_IDs1$Depth_cm <- paste0(Sample_data_IDs1$Depth, "cm")

Sample_data_IDs1$Site <- gsub("GCrew", "GCW", Sample_data_IDs1$Site)
Sample_data_IDs1$Site <- gsub("Gcrew", "GCW", Sample_data_IDs1$Site)
Sample_data_IDs1 <- Sample_data_IDs1[!is.na(Sample_data_IDs1$Sample.Name), ]

#Create IDs from what was collected for comparison later
Sample_data_IDs1 <- Sample_data_IDs1 %>%
  mutate(Sample_ID = paste(Site, Date_Run, Zone, Rep, Depth, Tree, sep = "_"))

Sample_data_IDs1$Sample_ID <- gsub("__", "_", Sample_data_IDs1$Sample_ID)
Sample_data_IDs1$Sample_ID <- gsub("m_NA", "m", Sample_data_IDs1$Sample_ID)
Sample_data_IDs1$Sample_ID <- gsub("m_", "m", Sample_data_IDs1$Sample_ID)
Sample_data_IDs1$Sample_ID <- gsub("_0cm", "", Sample_data_IDs1$Sample_ID)
...
Sample_data_IDs1 <- Sample_data_IDs1 %>%
  mutate(
    Type = case_when(
      str_detect(Rep, "Lys") ~ "Lys", 
      str_detect(Rep, "Sip") ~ "Sip", 
      str_detect(Rep, "PPR") ~ "PPR", 
      str_detect(Rep, "RHZ") ~ "RHZ", 
      Zone == "SW" ~ "SW", 
      TRUE ~ NA))

Sample_data_IDs1$Rep <- gsub("Lys", "", Sample_data_IDs1$Rep)
Sample_data_IDs1$Rep <- gsub("Sip", "", Sample_data_IDs1$Rep)
Sample_data_IDs1$Rep <- gsub("PPR", "", Sample_data_IDs1$Rep)
Sample_data_IDs1$Rep <- gsub("RHZ", "", Sample_data_IDs1$Rep)
...
sulfide_metadata_selected <- sulfide_metadata %>%
  dplyr::select(H2S_ID, Day, Time..24hr., Time.Zone_EDT.EST,  Field.Notes) 

#merge metadata with sample run data 
merged_data <- all_data_selected %>%
  left_join(sulfide_metadata_selected, by = c("Sample_ID" = "H2S_ID"))
...
final_data <- final_data %>%
  ...
  dplyr::select(
    Project, Region, Site, Zone, Replicate, Depth_cm, 
         ...
         Analysis_rundate, Run_notes, Field_notes, Type
