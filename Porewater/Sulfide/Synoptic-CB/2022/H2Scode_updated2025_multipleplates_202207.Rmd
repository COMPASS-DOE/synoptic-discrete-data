...
Some high CV, adl, and bdl samples. 
5/6 spikes were bad. 
No dups. 
...
file_paths_tidydata <- list.files(path = "Raw Data", pattern = "202207_Plate", full.names = TRUE)
head(file_paths_tidydata)

#file path name for processed data
#Write out final data frame 
final_data_path <- "Processed Data/COMPASS_H2S_202207_Plates_Combined.csv"
HighCV_path <- "Processed Data/COMPASS_H2S_202207_Plates_HighCVs.csv"
ADL_not_fixed_path <- "Processed Data/COMPASS_H2S_202207_Plates_ADL.csv"
BDL_not_fixed_path <- "Processed Data/COMPASS_H2S_202207_Plates_BDL.csv"
...
Sample_data1 <- Sample_data %>%
  mutate(IDs = gsub("DILx50", "", IDs)) %>%
  mutate(IDs = gsub("7 ", "7_", IDs)) %>%
  mutate(IDs = gsub("Tree_", "Tree", IDs)) %>%
  mutate(IDs = gsub(" ", "", IDs)) %>%
  rename(
    H2S_Conc_flag = H2S_flag,
    H2S_QAQC_flag = QAQC_flag)%>%
  separate(col = IDs, into = c("Number", "Date", "Site", "Zone", "Replicate", "Depth"), sep = "_") %>%
