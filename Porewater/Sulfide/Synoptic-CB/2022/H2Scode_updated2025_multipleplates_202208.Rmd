...
Run_notes="Plate2 std curve used. 
Some adl samples. 
2/5 plates had bad MC 10 and 1/5 plates had bad MC 20. 
3/7 spikes were bad. 
No dups. 
Some samples high CV, adl, bdl"#any notes from run
...
New_ID_1 <- "2_Gcrew 202208_UP_LysB_10cm"

Old_ID_2 <- "4_Gcrew_ 202208_UP__LysB_45cm"
New_ID_2 <- "4_Gcrew_202208_UP_LysB_45cm"
...
file_paths_tidydata <- list.files(path = "Raw Data", pattern = "202208_Plate", full.names = TRUE)
head(file_paths_tidydata)

#file path name for processed data
#Write out final data frame 
final_data_path <- "Processed Data/COMPASS_H2S_202208_Plates_Combined.csv"
HighCV_path <- "Processed Data/COMPASS_H2S_202208_Plates_HighCVs.csv"
ADL_not_fixed_path <- "Processed Data/COMPASS_H2S_202208_Plates_ADL.csv"
BDL_not_fixed_path <- "Processed Data/COMPASS_H2S_202208_Plates_BDL.csv"
...
for(i in 1:length(plates)){
  ...
  MC10 <- subset(H2S_MC1, IDs == "MC: Spike 10ppt")
  MC20 <- subset(H2S_MC1, IDs == "MC: Spike 20ppt") 

  #10ppt matrix check
  if("MC: Spike 10ppt" %in% MC10$IDs){
  ...
  QAMC<-rbind(QAMC,list(Plate=plates[i],IDs="MC: Spike 10ppt
...
#write out a flag to the sample dataframe if the MC is Bad
  ...
  #Add p-value to dataframe
  QAMC<-rbind(QAMC,list(Plate=plates[i],IDs="MC: Spike 20ppt
...
Sample_data <- dat1 %>% 
  ...
  subset((!IDs %like% "Blank"))

Sample_data1 <- Sample_data %>%
  mutate(IDs = gsub("Gcrew2", "Gcrew_2", IDs)) %>%
  mutate(IDs = gsub("-", "_", IDs)) %>%
  mutate(IDs = gsub(" ", "_", IDs)) %>%
  rename(
    H2S_Conc_flag = H2S_flag,
    H2S_QAQC_flag = QAQC_flag)%>%
  separate(col = IDs, into = c("Number", "Site", "Date", "Zone", "Replicate", "Depth"), sep = "_") %>%
  mutate(Site = gsub("Gcrew",'GCW', Site)) %>% 
  mutate(Sample_ID = paste(Site, Date_Run, Zone, Replicate, Depth, sep = "_")) %>% 
  mutate(ID_full = NULL) %>%
  mutate(Sample_ID = gsub("_NA", "", Sample_ID)) %>% 
  mutate(Depth = ifelse(Zone == "SW", "0cm", Depth))
