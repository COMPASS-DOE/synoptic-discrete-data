---
title: "COMPASS_SynopticCB_Collate_Sulfide_2024_Data"
...
files <- list.files(path = ".", pattern = "\\Plates_Combined_Data.csv$", full.names = TRUE, recursive =  TRUE)
...
final_data <- all_data %>%
  mutate(Replicate = gsub("Lys", "", Replicate), 
         Replicate = gsub("LYS", "", Replicate)) %>%
  rename(
  ...
  dplyr::select(
    Project, Region, Site, Zone, Replicate, Depth_cm, 
         ...
         H2S_mean_uM, H2S_sd_uM, H2S_cv, H2S_Conc_flag, H2S_cv_flag, Dilution,
...
write.csv(final_data, "COMPASS_SynopticCB_Sulfide_2024.csv", row.names = FALSE)
