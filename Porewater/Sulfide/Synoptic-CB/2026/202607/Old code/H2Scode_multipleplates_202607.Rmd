...
author: "July 2026 Samples Ran on 20260720, 20260724"
date: "`r Sys.Date()`"
output: 
  pdf_document:
    toc: false
...
Run_notes="Used std curve from Plate 1. MC 20 from the first day were bad. 2/6 dups were bad. 5/6 spikes were bad. All samples are present in metadata."#any notes from run
...
file_paths_tidydata <- list.files(path = "Raw Data", pattern = "tidy.*\\.csv$", full.names = TRUE)
(file_paths_tidydata)

#file path name for processed data
#Write out final data frame 
final_data_path <- "COMPASS_H2S_202607_Plates_Combined.csv"
HighCV_path <- "COMPASS_H2S_202607_Plates_HighCVs.csv"
ADL_not_fixed_path <- "COMPASS_H2S_202607_Plates_ADL.csv"
BDL_not_fixed_path <- "COMPASS_H2S_202607_Plates_BDL.csv"

#QAQC log path
log_path="Tidy Data/Sulfide_STD_QAQC.csv"

#Metadata path
Raw_Metadata = "Tidy Data/COMPASS_SynopticCB_PW_SampleLog_2026.csv"
...
dat <- dat %>%
  mutate(Plate = str_extract(Plate, "\\d{4}\\d{2}\\d{2}_Plate[0-9]+(?:_reruns)?"))
...
all_data_selected <- Sample_data_IDs %>%
  dplyr::select(-c("ID_conc", "Depth", "ID_full", "IDs"))
