...
write.csv(all_dat, "COMPASS_SynopticCB_Sulfide_AllData.csv", row.names = FALSE)


#write out a csv of the metadata associated with the data: 
write.csv(metadat, "COMPASS_SynopticCB_Sulfide_Metadata.csv", row.names = FALSE)
...
Sulfide_avg_depth <- all_dat_dates %>%
  ...
  group_by(Site, Zone, Depth_cm) %>%
   dplyr::summarize(
     mean_H2S = mean(H2S_mean_uM, na.rm=TRUE), 
     sd_H2S = sd(H2S_mean_uM, na.rm=TRUE))

viz_h2s_plot_depth <- ggplot(Sulfide_avg_depth, aes(x = as.numeric(Depth_cm), y = mean_H2S, color = Zone, group = Zone)) +
  geom_point(size = 3) + 
  geom_errorbar(aes(ymin = mean_H2S - sd_H2S, ymax = mean_H2S + sd_H2S), width = 0.5, size = 1) + 
  geom_line(size = 1.5) + 
  ...
  theme_classic() +
  labs(x = "Sampling Depth (cm)", y = "Sulfide (uM)", title = "Sulfide by Depth") +
