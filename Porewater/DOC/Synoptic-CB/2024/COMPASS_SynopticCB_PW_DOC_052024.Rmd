...
###### Run information - PLEASE CHANGE
  ...
  Script_run_by = "Isabelle E. Van Benschoten" #Code user 
 # run_notes = "NPOC Check Standard remade during run as 60mgL, so checks are within range"  #any notes from the run
...
log_p <- read.csv(Log_path)
log_p <- log_p[ ,-c(1)]

#make sure they both have dates as dates 
log_p$run_date <- as.Date(log_p$run_date)
log_p$analyte <- as.character(log_p$analyte)
log_p$curve <- as.character(log_p$curve)
Slopes$run_date <- as.Date(Slopes$run_date)

# Filter to only rows in Slopes that are NOT already in log (by run_date + analyte)
new_rows <- anti_join(Slopes, log_p, by = c("run_date", "analyte"))

# Append the new, non-duplicate rows to log
log_p <- bind_rows(log_p, new_rows)

#plot the current slops with teh previous slopes 
Slopes_chk <- ggplot(log_p, aes(run_date, slope, col=curve)) +
  geom_point(size=4) + 
  geom_line() + 
