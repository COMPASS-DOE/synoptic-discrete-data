---
title: "COMPASS_SynopticCB_Collate_DOC_2024_Data"
...
viz_NPOC_plot <- ggplot(data_plotting_all, aes(x = months_factor, y = npoc_mgL, fill = Zone)) +
  ...
  theme_classic() +
  labs(x = " ", y = "NPOC (mg/L)", title = "Synoptic CB Porewater: NPOC 2024") +
  theme(
...
viz_TDN_plot <- ggplot(data_plotting_all, aes(x = months_factor, y = tdn_mgL, fill = Zone)) +
  ...
  theme_classic() +
  labs(x = " ", y = "TDN (mg/L)", title = "Synoptic CB Porewater: TDN 2024") +
  theme(
    panel.spacing = unit(.05, "lines"), 
    panel.border = element_rect(color = "black", fill = NA, linewidth = 1)) + 
  theme(text = element_text(size = 20))+ 
...
write.csv(final_data_lys, "COMPASS_SynopticCB_PW_DOC_2024.csv")
