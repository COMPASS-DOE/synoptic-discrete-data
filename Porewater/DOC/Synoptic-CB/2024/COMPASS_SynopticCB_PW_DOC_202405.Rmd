---
title: "COMPASS Synoptic CB Porewater: DOC"
...
###### Run information - PLEASE CHANGE
  Date_Run = "6/03/24"  #Date that instrument was run
  ...
  Second NPOC Curve did not look good. One duplicate out of range and some checks 
  below expected concentration, but not by much - run okay."  #any notes from the run
  ...
  chks_name = "Chk_Std_50mgC_2mgN"  #what did you name your check standards? 
...
#file path and name for raw summary data file 
    ...
    raw_file_name = "Raw Data/COMPASS_SynopticCB_PW_DOC_202405.txt"
...
#file path and name for raw all peaks file 
    ...
    raw_allpeaks_name = "Raw Data/COMPASS_SynopticCB_PW_DOC_202405_allpeaks.txt" 
...
#file path and name of processed data file 
    ...
    processed_file_name = "Processed Data/COMPASS_SynopticCB_PW_Processed_DOC_202405.csv" #example
...
string_to_find <- "202405_MSM_UP_LysA_10cm"
found <- any(sapply(dat_raw, function(x) any(grepl(string_to_find, x))))
...
dat_raw$sample_name_old <- dat_raw$sample_name

if (any(grepl("202405_MSM_UP_LysA_10", dat_raw$sample_name))) {
  dat_raw <- dat_raw %>%
    mutate(
      sample_name = case_when(
        grepl("202405_MSM_UP_LysA_", sample_name) ~ sub("LysA_", "LysB_", sample_name),
        grepl("202405_MSM_UP_LysB_", sample_name) ~ sub("LysB_", "LysC_", sample_name),
        grepl("202405_MSM_UP_LysC_", sample_name) ~ sub("LysC_", "LysA_", sample_name),
        TRUE ~ sample_name
      )
    )
}

dat_raw$sample_name_old <- NULL
```
...
#Plot data and change colors based on flags to check it: 
viz_c_plot <- ggplot(dat_flagged_id, aes(x = sample_name, y = npoc_raw, fill = Zone)) +
...
doc_metadata_selected <- doc_metadata %>%
  select(DOC_ID, Year, Month, Day, Depth_cm, Lysimeter, Time..24hr., Time.Zone_EDT.EST,  Field.Notes) 
