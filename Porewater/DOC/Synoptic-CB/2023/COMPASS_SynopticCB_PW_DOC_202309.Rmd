...
dat_raw <- dat_raw %>% 
  mutate(sample_name = gsub("50_202039_GWI_WC_LysB_20cm", "50_202039_GWI_WC_LysB_20cm_dup", sample_name)) %>% ##This one should be a dup
  mutate(sample_name = str_replace_all(sample_name, "202039", "202309")) %>%
  mutate(sample_name = str_remove(sample_name, "^[0-9]+_"))
  #filter(!(sample_name == "202308_SWH_UP_LysA_20cm" & npoc_raw == 0.03413))
  #  mutate(
  #  sample_name = if_else(
  #    str_detect(sample_name, "^202306_"),  # if it already starts with "202306_"
 #     sample_name,                          # keep as is
#      paste0("202306_", sample_name)        # otherwise add prefix
#    )
  #)
 # mutate(sample_name = if_else(sample_name == "SWH_202037_UPCON_LysA_20cm",
  #                            "SWH_202307_UPCON_LysA_20cm", sample_name) ) %>% 
#  mutate(sample_name = if_else(sample_name == "GCW_202307_TR_lysA_45cm",
 #                             "GCW_202307_TR_LysA_45cm", sample_name) )
...
dups <- dat_raw %>%  
  ...
  mutate(sample_name = str_replace_all(sample_name, "50_202039_GWI_WC_LysB_20cm", "50_202039_GWI_WC_LysB_20cm_dup")) %>% ##This one should be a dup
  mutate(sample_name = str_replace_all(sample_name, "202309_GCW_WC_LysA_20c_dup", "202309_GCW_WC_LysA_20cm_dup")) %>% ##This one should be a dup
...
colnames(IDs) <- c("Date" , "Site_Code","Zone", "Lysimeter", "Depth")
#head(IDs)
...
duplicated_rows <- dat_flagged_id %>% filter(duplicated(sample_name))
duplicated_rows
