...
###### Run information - PLEASE CHANGE
  Date_Run = "04/30/23"  #Date that instrument was run
  ...
  run_notes = "Standard Curve was assessed manually on the instrument.
    Check GCW SW samples, because the npoc values are 0.0000. 
  Some sample IDs are missing from metadata: 202304_MSM_UP_LysA_45cm, 202304_MSM_UP_LysB_45cm, 202304_GWI_UP_LysA_20cm, 
  202304_MSM_TR_LysA_45cm, 202304_MSM_TR_LysC_10cm, 202304_MSM_TR_LysC_20cm"  #any notes from the run
  ...
  chks_name = "3ppmN_50ppmC_Chk"  #what did you name your check standards? 
...
#file path and name for raw summary data file 
    ...
    raw_file_name = "Raw Data/TOCTN_COMPASS_Synoptic_TCTN_202304.txt"
...
#file path and name of processed data file 
    ...
    processed_file_name = "Processed Data/COMPASS_SynopticCB_PW_Processed_DOC_202304.csv" #example
...
#calculate the percent of check standards that are within the range based on the flag 
c_dups_percent <- (sum(QAdups$npoc_dups_cv_flag == "YES", na.rm = TRUE)/sum(!is.na(QAdups$npoc_dups_cv_flag)))*100
n_dups_percent <- (sum(QAdups$tdn_dups_cv_flag == "YES", na.rm = TRUE)/sum(!is.na(QAdups$tdn_dups_cv_flag)))*100
