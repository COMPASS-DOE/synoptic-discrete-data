...
###### Run information - PLEASE CHANGE
  Date_Run = "11/19/2022"  #Date that instrument was run
...
#file path and name for raw summary data file 
    raw_file_name = c("Raw Data/TOCTN_COMPASS_Synoptic_TCTN_202211_1.txt") # , "Raw Data/TOCTN_COMPASS_Synoptic_TCTN_202211_2.txt" )
...
#file path and name of processed data file 
    processed_file_name = "Processed Data/COMPASS_SynopticCB_PW_Processed_DOC_202211.csv"
...
#fix sample missign underscore
dat_raw <- dat_raw %>% 
  mutate(sample_name = gsub("LysB20cm", "LysB_20cm", sample_name)) 


#want date to be at the beginning for now 
#MSM_202211_UP_LysA_10cm --> 202209_GWI_UP_LysA_10cm
dat_raw <- dat_raw %>% 
...
ifelse(c_blks_percent >= chks_flag, ">50% of Carbon Blank concentrations are lower 25% quartile of samples",
       "<50% of Carbon blaks are lower 25% quartile of samples - REASSESS")
ifelse(n_blks_percent >= chks_flag, ">50% of Nitrogen Blank concentrations are lower 25% quartile of samples",
       "<50% of Nitrogen blaks are lower 25% quartile of samples - REASSESS")
...
colnames(dups) <- c('npoc_raw_dup', "tdn_raw_dup", 'sample_name')
...
ifelse(c_dups_percent >= chks_flag, ">50% of Carbon Duplicates have a CV <10%",
       "<50% of Carbon Duplicates have a CB <10% - REASSESS")
ifelse(n_dups_percent >= chks_flag, ">50% of Nitrogen Duplicates have a CV <10%",
       "<50% of Nitrogen Duplicates have a CB <10% - REASSESS")

#write out a flag to the sample dataframe if more than 50% of the dups have CVs out of range 
...
colnames(IDs) <- c("Date", "Site_Code", "Zone", "Replicate", "Depth") #202211_GWI_UP_LysA_10cm
#head(IDs)
