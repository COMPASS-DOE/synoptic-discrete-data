...
Old_ID_1 <- "SWH_202405_SWAMP_LysA_10"
New_ID_1 <- "SWH_202405_SWAMP_LysA_10cm"

#identify the files you want to read in 
  #read in as a list to accommodate multiple runs in a month
  NOx_files <- c("Raw Data/COMPASS_Synoptic_CB_202405_VNox_1.csv",
                 "Raw Data/COMPASS_Synoptic_CB_202405_VNox_2.csv",
                 "Raw Data/COMPASS_Synoptic_CB_202405_VNox_4.csv")
  NH3_PO4_files <- c("Raw Data/COMPASS_Synoptic_CB_202405_NH3_PO4_1.csv",
                     "Raw Data/COMPASS_Synoptic_CB_202405_NH3_PO4_2.csv")
...
# Define the file path for QAQC log file - NO Need to change just check year 
  ...
  final_path <- "Processed Data/COMPASS_Synoptic_Nutrients_202405.csv"

#record any notes about the run or anything other info here: 
  run_notes <- "Samples were put in the wrong vials, so 
  MSM_202405_UP_LysA_ was switched to MSM_202405_UP_LysB_, 
  MSM_202405_UP_LysB_ was switched to MSM_202405_UP_LysC_, 
  MSM_202405_UP_LysC_ was switched to MSM_202405_UP_LysA_.
  It looks like NH3 was only ran for GCW, MSM, and SWH. 
  It looks like PO4 was only ran for GCW and MSM. 
  Mean NOx Reduction Efficiency is less than 95% (94.18327%). 
  Most of the NOx blanks are above the lower 25% quartile of samples. 
  3 NOx dups and one NH3 dup have high CVs, but more than 60% have low CVs. 
  2 NOx spikes have high CVs, but more than 60% have low CVs."
...
string_to_find <- "MSM_UP_202405_LysA_10cm"
found <- any(sapply(df_all, function(x) any(grepl(string_to_find, x))))
...
df_all$Sample_Name_old <- df_all$Sample_Name

if (any(grepl("MSM_UP_202405_LysA_10", df_all$Sample_Name))) {
  df_all <- df_all %>%
    mutate(
      Sample_Name = case_when(
        grepl("MSM_UP_202405_LysA_", Sample_Name) ~ sub("LysA_", "LysB_", Sample_Name),
        grepl("MSM_UP_202405_LysB_", Sample_Name) ~ sub("LysB_", "LysC_", Sample_Name),
        grepl("MSM_UP_202405_LysC_", Sample_Name) ~ sub("LysC_", "LysA_", Sample_Name),
        TRUE ~ Sample_Name
      )
    )
}

df_all$Sample_Name_old <- NULL
```
...
## Analyze Duplicates
```{r Analyze Duplicates, echo=FALSE, warning=FALSE}
...
```{r Sample Identification, echo=FALSE, warning=FALSE}

cat("Sample Processing")

##MSM and GCW samples have Zone and then Samp_Time
##GWI and SWH samples have Samp_Time and then Zone

samples_MSM_GCW <- samples_flagged %>%
  filter(str_detect(Sample_Name, "MSM|GCW")) %>%
  filter(!str_detect(Sample_Name, "RHZ|PPR")) %>% 
  mutate(Run_Time = lubridate::mdy_hm(Run_Time)) %>%
  separate(
    col = Sample_Name,
    sep = "_",
    into = c("Site", "Zone", "YearMonth", "Replicate", "Depth"),
    remove = FALSE)

samples_GWI_SWH <- samples_flagged %>%
  filter(str_detect(Sample_Name, "GWI|SWH")) %>%
  filter(!str_detect(Sample_Name, "RHZ|PPR")) %>% 
  mutate(Run_Time = lubridate::mdy_hm(Run_Time)) %>%
  separate(
    col = Sample_Name,
...
samples_flagged_all <- rbind(samples_MSM_GCW, samples_GWI_SWH)
