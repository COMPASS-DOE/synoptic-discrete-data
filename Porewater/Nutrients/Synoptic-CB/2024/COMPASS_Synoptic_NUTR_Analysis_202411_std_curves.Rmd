...
#set the run date & user name 
  run_date <- "8/7/2025"
  sample_year <- "2024"
  sample_month <- "NOVEMBER"
  user <- "Isabelle Van Benschoten"
  
##Data entered incorrectly
Old_ID_1 <- "SWH_202411_SWAMP_LycC_45cm"
New_ID_1 <- "SWH_202411_SWAMP_LysC_45cm"

#identify the files you want to read in 
  #read in as a list to accommodate multiple runs in a month
  NOx_files <- c("Raw Data/COMPASS_Synoptic_CB_202411_VNOx_1.csv",
                 "Raw Data/COMPASS_Synoptic_CB_202411_VNOx_2.csv",
                 "Raw Data/COMPASS_Synoptic_CB_202411_VNOx_3.csv")
  NH3_PO4_files <- c("Raw Data/COMPASS_Synoptic_CB_202411_NH3_PO4_1.csv",
                     "Raw Data/COMPASS_Synoptic_CB_202411_NH3_PO4_2.csv",
                     "Raw Data/COMPASS_Synoptic_CB_202411_NH3_PO4_3.csv")
...
# Define the file path for QAQC log file - NO Need to change just check year 
  ...
  final_path <- "Processed Data/COMPASS_Synoptic_Nutrients_202411.csv"

#record any notes about the run or anything other info here: 
  run_notes <- "One PO4 check standard came back very high, all others were within range.
                A few duplicates were out of range but still within CV.
                SWH_202411_SWAMP_LycC_45cm, SWH_202411_TR_LysC_10cm, 
                MSM_202411_TR_LysC_10cm, MSM_202411_TR_LysC_45cm were missing from metadata. "
...
####Fix Sample IDs####
df_all$Sample_Name <- str_replace(df_all$Sample_Name, Old_ID_1, New_ID_1)
...
fit_quadratic_model <- function(data_subset) {
      lm(y ~ x + I(x^2), data = data_subset)
}

stds_NOx$y <- stds_NOx$Conc
stds_NOx$x <- stds_NOx$Absorbance

quad_reg_NOx <- stds_NOx %>%
      group_by(Run_Date) %>%
      nest()%>%
      mutate(model = map(data, fit_quadratic_model))

std_curve_NOx_results <- quad_reg_NOx %>%
  mutate(
    coefficients = map(model, tidy),
    R_squared = map_dbl(model, ~glance(.)$r.squared)
  ) %>%
  unnest(coefficients) %>%
  select(Run_Date, term, estimate, R_squared) %>%
  pivot_wider(names_from = term, values_from = estimate) %>%
  rename(Intercept = `(Intercept)`, Slope = x)%>%
  select(Run_Date, R_squared, Intercept, Slope)%>%
  mutate(Test = "NOx")
...
#NH3
fit_linear_model <- function(data_subset) {
      lm(y ~ x, data = data_subset)
}

stds_NH3$y <- stds_NH3$Conc
stds_NH3$x <- stds_NH3$Absorbance

lin_reg_NH3 <- stds_NH3 %>%
      group_by(Run_Date) %>%
      nest()%>%
      mutate(model = map(data, fit_linear_model))

std_curve_NH3_results <- lin_reg_NH3 %>%
  mutate(
    coefficients = map(model, tidy),
    R_squared = map_dbl(model, ~glance(.)$r.squared)
  ) %>%
...
stds_PO4$y <- stds_PO4$Conc
stds_PO4$x <- stds_PO4$Absorbance

lin_reg_PO4 <- stds_PO4 %>%
      group_by(Run_Date) %>%
      nest()%>%
      mutate(model = map(data, fit_linear_model))

std_curve_PO4_results <- lin_reg_PO4 %>%
  mutate(
    coefficients = map(model, tidy),
    R_squared = map_dbl(model, ~glance(.)$r.squared)
  ) %>%
...
std_curve_combo <- bind_rows(std_curve_NOx_results, std_curve_NH3_results, std_curve_PO4_results)
...
std_curve_NOx <- ggplot(stds_NOx, aes(x = Conc, y = Absorbance)) +
  ...
  stat_cor(aes(label = after_stat(rr.label)), r.digits = 4, color = "black", geom = "label", 
           label.x = 0.25, label.y = max(stds_NOx$Absorbance), size = 4)
print(std_curve_NOx)
...
std_curve_NH3 <- ggplot(stds_NH3, aes(x = Conc, y = Absorbance)) +
  ...
  stat_cor(aes(label = after_stat(rr.label)), r.digits = 4, color = "black", geom = "label", 
           label.x = 0.25, label.y = max(stds_NH3$Absorbance), size = 4)
print(std_curve_NH3)
...
std_curve_PO4 <- ggplot(stds_PO4, aes(x = Conc, y = Absorbance)) +
  ...
  stat_cor(aes(label = after_stat(rr.label)), r.digits = 4, color = "black", geom = "label", 
           label.x = 0.08, label.y = max(stds_PO4$Absorbance), size = 4)
print(std_curve_PO4)
...
#Write out to the user whether or not the r2 is above the cutoff of 0.98
  ifelse(std_curve_NOx_results$R_squared <= r2_cutoff, 
         "NOx Curve r2 is below cutoff! - REASSESS", 
         "NOx Curve r2 GOOD - PROCEED")
  ifelse(std_curve_NH3_results$R_squared <= r2_cutoff, 
         "NH3 Curve r2 is below cutoff! - REASSESS", 
         "NH3 Curve r2 GOOD - PROCEED")
  ifelse(std_curve_PO4_results$R_squared <= r2_cutoff, 
...
add_log <- as.data.frame(bind_rows(std_curve_NOx_results, std_curve_NH3_results, std_curve_PO4_results))
add_log <- add_log %>%
  rename(run_date = Run_Date)
add_log$user <- user
...
####Need to fix this so it checks each R squared separately and only flags the ones that are from that run date with a low R squared!####

df_all <- df_all %>%
  mutate(
    NOx_flag = if (mean(std_curve_NOx_results$R_squared) <= r2_cutoff) {
      "NOx r2 low"
    } else {
      ""
    },
    NH3_flag = if (mean(std_curve_NH3_results$R_squared) <= r2_cutoff) {
      "NH3 r2 low"
    } else {
      ""
    },
    PO4_flag = if (mean(std_curve_PO4_results$R_squared) <= r2_cutoff) {
      "PO4 r2 low"
    } else {
      ""
    }
...
df_duplicates <- df_all %>%
  ...
  filter(
    ...
    !startsWith(Sample_Name, "AbsChk_") &
