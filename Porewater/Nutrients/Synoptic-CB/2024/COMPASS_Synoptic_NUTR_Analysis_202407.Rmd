...
#record any notes about the run or anything other info here: 
  run_notes <- "Reduction Efficiency was not ran for all of the runs included within this month.
                One NH3 check standard had high CV. 
                One PO4 blank had high CV. 
                Over 60% of PO4 Duplicates have a CV less than 10%. 
                Some sample IDs are missing from metadata: 
                SWH_202407_UP_LysC_10cm, SWH_202407_WC_LysA_45cm" 
...
library(textclean)
df_all <- df_all %>%
  mutate(Sample_Name = replace_non_ascii(str_trim(Sample_Name)))%>%
  mutate(Run_Number= replace_non_ascii(str_trim(Run_Number)))%>%
  mutate(Conc= replace_non_ascii(str_trim(Conc)))%>%
  mutate(Absorbance = replace_non_ascii(str_trim(Absorbance)))%>%
  mutate(Dilution = replace_non_ascii(str_trim(Dilution)))%>%
  mutate(Unit = replace_non_ascii(str_trim(Unit)))%>%
  mutate(Test = replace_non_ascii(str_trim(Test)))%>%
  mutate(Run_Time = replace_non_ascii(str_trim(Run_Time)))


df_all <- df_all %>%
  mutate(
    Sample_Name = as.character(Sample_Name),
    Run_Number = as.numeric(Run_Number),
