...
Old_ID_1 <- "SWH_202410_SWAMP_LycC_45cm"
New_ID_1 <- "SWH_202410_SWAMP_LysC_45cm"

Old_ID_2 <- "SWH_202409_WC_LysA_10cm"
New_ID_2 <- "SWH_202410_WC_LysA_10cm"
...
#identify the files you want to read in 
  ...
  NH3_PO4_files <- c("Raw Data/COMPASS_Synoptic_CB_202410_NH3_PO4_1.csv")
...
# Define the file path for QAQC log file - NO Need to change just check year 
  ...
  final_path <- "Processed Data/COMPASS_Synoptic_Nutrients_202410.csv"

#record any notes about the run or anything other info here: 
  run_notes <- "Some sample IDs are missing from metadata: SWH_202410_UP_LysC_10cm
  Nitrate and nitrite reduction efficiency is okay, graph off 
  because only one run for this month. 
  Performance check for NOx looks like DI was run instead of the peCheck.
  ** Something up with the phosphate on this run... needs looked at. "
...
cols_to_remove <- c("RUNSTARTED", "X.4", "X.6", "X.7", "X.10")
...
df_all$Sample_Name[df_all$Sample_Name == Old_ID_2] <- New_ID_2
