...
##Data entered incorrectly if it is only a few 
Old_ID_1 <- "wrong_sample_name"
New_ID_1 <- "correct_sample_name"

#identify the files you want to read in 
#read in as a list to accommodate multiple runs in a month
NOx_files <- c("Raw Data/SEAL_COMPASS_Synoptic_NOx_2026XX_1.csv",
               "Raw Data/SEAL_COMPASS_Synoptic_NOx_2026XX_2.csv",
               "Raw Data/SEAL_COMPASS_Synoptic_NOx_2026XX_3.csv")
NH3_PO4_files <- c("Raw Data/SEAL_COMPASS_Synoptic_NH3_PO4_2026XX_1.csv",
                   "Raw Data/SEAL_COMPASS_Synoptic_NH3_PO4_2026XX_2.csv", 
                   "Raw Data/SEAL_COMPASS_Synoptic_NH3_PO4_2026XX_3.csv")

# Define the file path for QAQC log file - NO Need to change just check year 
file_path <- "Raw Data/SEAL_COMPASS_Synoptic_QAQC_Log_2026.csv"
final_path <- "Processed Data/COMPASS_Synoptic_Nutrients_2026XX.csv"
...
```{r qaqc code setup, include=FALSE}

##################################################################################################
#SET UP CHUNK  - Check the values here, but you shouldn't need to change anything
#################################################################################################
...
##################################################################################################
#METADATE  - Read in metadata and create similar sample IDs for matching to samples 
#################################################################################################

#link directory to google drive location
directory <- "https://docs.google.com/spreadsheets/d/1HCAN0_q6y17x0RUXVzID09hVal-RfwWc/edit?gid=495553220#gid=495553220"

#Download sample log to Raw Data folder
drive_download(
  file = directory,
  path = "Raw Data/COMPASS_SynopticCB_PW_SampleLog.xlsx",
  overwrite = TRUE
)

sheet_names <- excel_sheets("Raw Data/COMPASS_SynopticCB_PW_SampleLog.xlsx")
sheet_names

raw_metadata <- read_excel("Raw Data/COMPASS_SynopticCB_PW_SampleLog.xlsx", sheet = paste0(sample_year, "_Log"))
...
**Run date:** `r run_date`  
**Run by:** `r user`  
**Samples processed:** `r sample_month` `r sample_year`  
**Run notes:** `r run_notes`

**NOx files used:**

- `r basename(NOx_files[1])`
- `r basename(NOx_files[2])`
- `r basename(NOx_files[3])`

**NH3/PO4 files used:**

- `r basename(NH3_PO4_files[1])`
- `r basename(NH3_PO4_files[2])`
- `r basename(NH3_PO4_files[3])`
...
## Fix any incorrectly entered sample data 
```{r, include=FALSE}

###########################################################################################
# Fix entered errors - you may need to change things here if there were entered errors 
###########################################################################################
...
#############################################################################
# Extract standards from file - should not need to make any changes here
#############################################################################
...
```{r Assess Standard Curves, echo=FALSE, comment="", fig.pos="H"}
#########################################################################
## Plot standards data - should not need to make any changes here
#########################################################################
...
std_curve_PO4 <- ggplot(stds_PO4, aes(x = Conc, y = Absorbance)) +
  geom_point(size=3) +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, color = "darkkhaki") +
  facet_wrap(~Run_Date) +
  theme_bw() + 
  labs(x="Concentration (ppm)",
       y="Absorbance",
...
#Write out to the user whether or not the r2 is above the cutoff of 0.98
  cat(ifelse(std_curve_NOx_1$R_squared <= r2_cutoff, 
         "NOx Curve r2 is below cutoff! - REASSESS", 
         "NOx Curve r2 GOOD - PROCEED"), "\n")
  cat(ifelse(std_curve_NH3_1$R_squared <= r2_cutoff, 
         "NH3 Curve r2 is below cutoff! - REASSESS", 
         "NH3 Curve r2 GOOD - PROCEED"), "\n")
  cat(ifelse(std_curve_PO4_1$R_squared <= r2_cutoff, 
...
Slopes_chk <- ggplot(log, aes(run_date, Slope, col=Test)) + 
  ...
  geom_point(data = add_log, size=4, color = "black")
Slopes_chk

#averaging the slopes from the log and printing
avg_slopes <- log %>%
  group_by(Test) %>%
  summarise(Avg_Past_Slopes = mean(Slope, na.rm = TRUE))


combined_results <- std_curve_combo %>%
  left_join(
    avg_slopes %>% select(Test, Avg_Past_Slopes),
    by = "Test"
  )
...
## Dilution Checks - If a sample was diluted, ensure we are using the diluted value
```{r Dilution Corrections, echo=FALSE, comment=""}

#########################################################################
## Check Diluted Sample info - should not need to make any changes here
#########################################################################
...
#make the dups a dataframe 
dil_dups <- df_duplicates %>%
  distinct(Sample_Name) %>%
  select(Sample_Name)
...
################################################################################################
## Check how well your third party (peChk) came out - should not need to make any changes here
################################################################################################
...
#Results of peCheck analysis 
check_results <- data.frame(
  Analyte = c("NOx", "NH3", "PO4"),
  `Run mean (ppm)` = c(
    NOx_peChk$Mean_Chk_Conc,
    NH3_peChk$Mean_Chk_Conc,
    PO4_peChk$Mean_Chk_Conc),
  `Expected concentration (ppm)` = c(
    NOx_pe,NH3_pe,PO4_pe),
  `chk_flag` = c(
    ifelse(
      NOx_peChk$pct_diff_Chk >= chk_flag,
      "% Difference >25% - REASSESS",
      "% Difference <25% - PROCEED"
    ),
    ifelse(
      NH3_peChk$pct_diff_Chk >= chk_flag,
      "% Difference >25% - REASSESS",
      "% Difference <25% - PROCEED"
    ),
    ifelse(
      PO4_peChk$pct_diff_Chk >= chk_flag,
      "% Difference >25% - REASSESS",
      "% Difference <25% - PROCEED")))
...
knitr::kable(
  ...
  col.names = c(
    ...
    "Third Party Check flag"
  ),
  digits = 2,
  align = "cccc",
  caption = "Check Standard Results"
)
...
## Assess NOx Reduction Efficiency 
```{r Assess reduction Efficiency, echo=FALSE, comment="", fig.pos="H"}

############################################################################################################
## Check your REFF tests for reduction efficiency of NO3 to NO2 - should not need to make any changes here
############################################################################################################
...
cat(ifelse(NOx_mean_red_eff >= 95,
       "Mean NOx Reduction Efficiency >95% - PROCEED",
       "Mean NOx Reduction Efficiency <95% - REASSESS"),
  "\n") #the \n here adds a line after the reported info 

cat("Mean Reduction Efficiency:", NOx_mean_red_eff, "\n")
...
```{r Check Standards, echo=FALSE, comment="", fig.pos="H"}

################################################################################################
## Assess your CCV / Check Standards - should not need to make any changes here
################################################################################################
...
cat(ifelse(NOx_chks_percent >= chks_flag, 
...
cat(ifelse(NH3_chks_percent >= chks_flag,
...
cat(ifelse(PO4_chks_percent >= chks_flag,
...
## Analyze Blanks 
```{r Analyze Blanks, echo=FALSE, comment="", fig.pos="H"}

################################################################################################
## Assess your CCBs / Blanks through the run - should not need to make any changes here
################################################################################################
...
#report out if flags indicate need for rerun
cat(ifelse(NOx_blks_percent >= 60,
...
cat(ifelse(NH3_blks_percent >= 60,
...
cat(ifelse(PO4_blks_percent >= 60,
...
## Analyze Duplicates
```{r Analyze Duplicates, echo=FALSE, warning=FALSE, comment="", fig.pos="H"}

################################################################################################
## Assess your duplicates through the run - should not need to make any changes here
################################################################################################
...
NOx_dups_plot <- ggplot(data =Nox_dup_chk, aes(x =Pair_ID, y =CV, fill=NOx_diff_flag)) +
        geom_bar(stat = 'identity') + 
        theme_classic() + 
        labs(x= "Sample ID", y="CV (%)", title = "NOx Duplicates") + 
...
NH3_dups_plot <- ggplot(data = NH3_dup_chk, aes(x =Pair_ID, y =CV, fill=NH3_diff_flag)) +
        geom_bar(stat = 'identity') + 
        theme_classic() + 
        labs(x= "Sample ID", y="CV (%)", title = "NH3 Duplicates") + 
...
PO4_dups_plot <- ggplot(data =PO4_dup_chk, aes(x =Pair_ID, y =CV, fill=PO4_diff_flag)) +
       geom_bar(stat = 'identity') + 
       theme_classic() + 
        labs(x= "Sample ID", y="CV (%)", title = "PO4 Duplicates") + 
       scale_fill_manual(values = c("YES" = "darkslateblue", "NO, rerun" = "darkred")) +
       theme(legend.position="none") +  
       geom_hline(yintercept=10, linetype="dashed", 
                color = "black", linewidth=1) + 
...
## Spikes
```{r Analyze Spikes, echo=FALSE, comment="", fig.pos="H"}

################################################################################################
## Assess your spikes through the run - should not need to make any changes here
################################################################################################
...
#spk flag report out
cat(ifelse(NOx_spk_percent >= chks_flag, 
       ">60% of Spikes have a CV <50% - PROCEED",
       "<60% of Carbon Spikes have a CV <50% - REASSESS"), "\n")
cat(ifelse(NH3_spk_percent >= chks_flag, 
       ">60% of Spikes have a CV <50% - PROCEED",
       "<60% of Carbon Spikes have a CV <50% - REASSESS"), "\n")
cat(ifelse(PO4_spk_percent >= chks_flag, 
       ">60% of Spikes have a CV <50% - PROCEED",
       "<60% of Carbon Spikes have a CV <50% - REASSESS"), "\n")
...
NOx_spk_plot <- ggplot(data =df_spk_NOx_chk, aes(x =Pair_ID, y =CV, fill=Spike_diff_flag)) +
        geom_bar(stat = 'identity') + 
        theme_classic() + 
        labs(x= "Sample ID", y="CV (%)", title = "NOx Auto Spikes") + 
...
NH3_spk_plot <- ggplot(data = df_spk_NH3_chk, aes(x =Pair_ID, y =CV, fill=Spike_diff_flag)) +
        geom_bar(stat = 'identity') + 
        theme_classic() + 
        labs(x= "Sample ID", y="CV (%)", title = "NH3 Auto Spikes") + 
        scale_fill_manual(values = c("YES" = "darkcyan", "NO, check" = "darkred")) +
        theme(legend.position="none")  + 
        geom_hline(yintercept=50, linetype="dashed", 
            color = "black", linewidth=1)  +
...
PO4_spk_plot <- ggplot(data =df_spk_PO4_chk, aes(x =Pair_ID, y =CV, fill=Spike_diff_flag)) +
       geom_bar(stat = 'identity') + 
       theme_classic() + 
        labs(x= "Sample ID", y="CV (%)", title = "PO4 Auto Spikes") + 
       scale_fill_manual(values = c("YES" = "darkcyan", "NO, check" = "darkred")) +
       theme(legend.position="none")  + 
       geom_hline(yintercept=50, linetype="dashed", 
            color = "black", linewidth=1)  +
...
## Matrix Effects
```{r Analyze Matrix Effects, echo=FALSE, comment="", fig.pos="H"}

################################################################################################
## Assess Abs_Chks for matrix effects - should not need to make any changes here
################################################################################################

cat("Run either a 10ppt ASW or a 10ppt and a 20ppt ASW to assess matrix effects due to salinity")
...
## Add unit converted cata column (convert mg/L to uM )
...
# Concentration / Dilution Flags

## Sample Concentration Flag - mark sample as within or out of range
...
## Sample processing: pull out sample id 
```{r Sample Identification, echo=FALSE, warning=FALSE, comment=""}

################################################################################################
## Pulling out relevant info to compare to metadata - should not need to make any changes here
################################################################################################
...
samples_flagged <- samples_flagged %>%
  ...
  separate(
    col = Sample_Name,
    sep = "_",
    into = c("Site", "YearMonth", "Zone", "Replicate", "Depth"),
    remove = FALSE) 


##Remake the sample names
#Example: GCW_202404_WC_LysA_10cm
samples_flagged_all <- samples_flagged %>%  
...
## Check Sample IDs with Metadata / Sample log and merge metadata
```{r check sample ids with metadata, echo=FALSE, warning=FALSE, comment=""}

######################################################################################################
## Check that samples are listed in metadata & merge info - should not need to make any changes here
######################################################################################################

#check to see if all samples are present in the metadata 
all_present <- all(samples_flagged_all$Sample_Name %in% nutr_metadata$NUTR_ID)
...
if (all_present) {
  ...
  missing_ids <- setdiff(samples_flagged_all$Sample_Name, nutr_metadata$NUTR_ID)
  print(missing_ids)
}
...
merged_data <- samples_flagged_all %>%
...
# Visualize Data
##NOx Samples
```{r Visualize NOx Data, echo=FALSE, fig.width=10, fig.height=5, out.width="100%", fig.pos="H"}

######################################################################################################
## Plot data for initial check (different for each project) - should not need to make any changes here
######################################################################################################
...
```
##NH3 Samples
```{r Visualize NH3 Data, echo=FALSE, fig.width=10, fig.height=5, out.width="100%", fig.pos="H"}
...
```
##PO4 Samples 
```{r Visualize PO4 Data, echo=FALSE, fig.width=10, fig.height=5, out.width="100%", fig.pos="H"}
...
######################################################################################################
## Clean up and export processed data - should not need to make any changes here
######################################################################################################
