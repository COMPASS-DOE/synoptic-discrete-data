---
title: "COMPASS_Synoptic_SEAL_Data_Analysis_July2022"
...
##Run notes
```{r}
Run_notes = "Several GCrew samples were duplicated without a dilution for Vanadium NOx, appears to be instrumentation issue with reruns.
            Subsetted dataframe in this code to remove samples run orginially and keep the reruns."
```
...
#QAQC 
  #R2 and Slopes 
#These are checked for drift or differences in this code 

  #Duplicates and Spikes 
#This is checked by the SEAL software 
#If 80% of the dups and spikes are within range we accept 
#the only exceptions are NOx values that are below the detection limit - we do not count dups out of range that are below the detection limit 
#these are run rouhgly every 10-12 samples per tray. 

  #CCV and CCBs 
#CCV's are 50% of the top standard checks run every 10 samples, these are checked by the softare; if 80% are within range we accept the tray 
#CCB's are DI blanks run every 10 samples, these are checked by the software; if 80% are within range we accept the tray 

  #Third Party Standard
#This is checked within the code - must be within 20% of the peChk concentration 
...
#read in data 
file1 <- read.csv("Raw Data/SEAL_COMPASS_Synoptic_NH3_PO4_202207_1.csv")
...
#read in data 
file2 <- read.csv("Raw Data/SEAL_COMPASS_Synoptic_NH3_PO4_202207_2.csv")
#Quick look at dataframe
head(file2)
#take out only the columns that we need 
...
#read in data 
file3 <- read.csv("Raw Data/SEAL_COMPASS_Synoptic_NH3_PO4_202207_3.csv")
#Quick look at dataframe
head(file3)
#take out only the columns that we need 
...
# GWI_TR_RHZ_SF_8 second run was much higher than any other samples in that Site and Zone, so remove that one. 
alldat3 <- alldat3 %>% 
  filter(!(Sample_Name == "GWI_TR_RHZ_SF_8" & Test == "o-PHOS 0.3" & Conc == "0.765889"))
...
#read in data 
Nfile1 <- read.csv("Raw Data/SEAL_COMPASS_Synoptic_NOx_202207_1.csv")
#Quick look at dataframe
head(Nfile1)

library(lubridate)
#This run was stopped and restarted... need to take out the rows that are not needed: 
Nfile1 <- Nfile1 %>%
  mutate(X.8 = mdy_hm(X.8)) %>%        # parse timestamp
  filter(format(X.8, "%H:%M") >= "11:26")
...
#read in data 
Nfile2 <- read.csv("Raw Data/SEAL_COMPASS_Synoptic_NOx_202207_2.csv")
#Quick look at dataframe
head(Nfile2)
#take out only the columns that we need 
...
#read in data 
Nfile3 <- read.csv("Raw Data/SEAL_COMPASS_Synoptic_NOx_202207_3.csv")
#Quick look at dataframe
head(Nfile3)
#take out only the columns that we need 
...
#pull out peChecks from alldat and Nalldat 
chks <- alldat[alldat$Sample_Name %like% "peCheck", ]
head(chks)

Nchks <- Nalldat[Nalldat$Sample_Name %like% "peCheck", ]

NH4chk <- subset(chks, Test == "Ammonia 2")
NH4chk$ConcTrue <- 0.948
NH4chk$Conc <- as.numeric(NH4chk$Conc)
NH4chk$Chk_diff <- (NH4chk$Conc - NH4chk$ConcTrue)/((NH4chk$Conc+NH4chk$ConcTrue)/2) * 100
NH4chk$Chk_dff_flag <-  ifelse(NH4chk$Chk_diff <25, 'YES', 'NO, rerun')
head(NH4chk)

PO4chk <- subset(chks, Test == "o-PHOS 0.3")
PO4chk$ConcTrue <- 0.818
PO4chk$Conc <- as.numeric(PO4chk$Conc)
PO4chk$Chk_diff <- (PO4chk$Conc - PO4chk$ConcTrue)/((PO4chk$Conc+PO4chk$ConcTrue)/2) * 100
PO4chk$Chk_dff_flag <-  ifelse(PO4chk$Chk_diff <25, 'YES', 'NO, rerun')
head(PO4chk)

NOXchk <- subset(Nchks, Test == "Vanadium NOx")
NOXchk$ConcTrue <- 0.706
NOXchk$Conc <- as.numeric(NOXchk$Conc)
NOXchk$Chk_diff <- (NOXchk$Conc - NOXchk$ConcTrue)/((NOXchk$Conc+NOXchk$ConcTrue)/2) * 100
NOXchk$Chk_dff_flag <-  ifelse(NOXchk$Chk_diff <25, 'YES', 'NO, rerun')
head(NOXchk)

Allchks <- rbind(NH4chk, PO4chk)
Allchks <- rbind(Allchks, NOXchk)
head(Allchks)

#plot dups output as a bar graph to easily check - want any over 10% to be red need to work on this 
Chksbar <- ggplot(data = Allchks, aes(x = Sample_Name, y = Chk_diff, fill=Test)) +
        geom_bar(stat = 'identity') + 
        facet_wrap(~Test) +
        theme_classic() + labs(x= "Sample ID", y="peChecks Percent Difference") + 
...
##Not sure why these were ran again on the SEAL without dilutions: 
# GWI_TR_RHZ_SF_8, GWI_WC_SipA_20cm, GWI_WC_SipC_45cm
...
## NOx Dilution Corrections - ensure the latest dilution is kept
```{r NOx Dilution Corrections, echo=FALSE}
...
NH4_pull <- NH4samples[ ,c(2,3,5,11) ]
head(NH4_pull)

PO4_pull <- PO4samples[ ,c(2,3,5,11) ]
head(PO4_pull)

NOX_pull <- NOXsamples[ ,c(2,3,5,12) ]
head(NOX_pull)
...
colnames(all_data) <- c("Sample_Name", "NH3_mgL", "NH3_dil", "NH3_uM", "PO4_mgL", "PO4_dil", "PO4_uM", "NOx_mgL", "NOx_dil", "NOx_uM")
head(all_data)
...
all_data$NH3_range <- ifelse(all_data$NH3_mgL/all_data$NH3_dil<0.02, "bdl",  ifelse(all_data$NH3_mgL/all_data$NH3_dil>2, "adl", "Within_Range")) 
all_data$PO4_range <- ifelse(all_data$PO4_mgL/all_data$PO4_dil<0.003, "bdl",  ifelse(all_data$PO4_mgL/all_data$PO4_dil>3, "adl", "Within_Range")) 
all_data$NOx_range <- ifelse(all_data$NOx_mgL/all_data$NOx_dil<0.025, "bdl",  ifelse(all_data$NOx_mgL/all_data$NOx_dil>1, "adl", "Within_Range"))
head(all_data)
```
...
out <- all_data[ ,c(1,2,4,5,7,8,10,11,12,13)]
# out <- all_data
head(out)
...
colnames(IDs) <- c("Site" ,"Zone", "Replicate", "Depth", "Tree")
IDs$Date <- 202207
IDs$Month <- "July"
head(IDs)
...
alldat <- alldat %>% 
  filter(!Sample_Name == "GCrew_WC_SipA_45cm_2")
...
write.csv(alldat, file="Processed Data/COMPASS_SynopticCB_Nutrients_202207.csv")
