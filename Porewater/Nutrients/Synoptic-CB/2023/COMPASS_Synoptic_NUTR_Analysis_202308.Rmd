...
#record any notes about the run or anything other info here: 
  run_notes <- "NH3 peCheck is out of range, but other run metrics are good,
  so we are accepting this run."
...
##Fix RHZ Sample Naming different across analytes
df_all <- df_all %>% #MSM_202308_RHZ_TR_SF_1
  separate(
    col = Sample_Name,
    sep = "_",
    into = c("Site", "Samp_Time", "Zone", "Replicate", "Depth", "Number"),
    remove = FALSE) %>%
  mutate(Zone = ifelse(Zone == "RHZ", Replicate, Zone)) %>%
  mutate(Sample_Name = ifelse(str_detect(Sample_Name, "RHZ"), paste(Site, Samp_Time, Zone, "RHZ", Depth, Number, sep = "_"), Sample_Name)) %>%
  select(-c("Site", "Samp_Time", "Zone", "Replicate", "Depth", "Number"))
...
```{r}
df_all <- df_all %>% 
  mutate(Sample_Name = gsub("SIp", "Sip", Sample_Name), 
         Sample_Name = gsub("202038", "202308", Sample_Name))
```
...
df_PO4_dup <- df_all_cor %>%
  filter(Test == "o-PHOS 0.3") %>%
  filter(
     !grepl("CCV|CCB|Standard|Auto Spike|Blank|1mg/L ammonia|Abs_Chk_20ppt|Abs_Chk_10ppt|0.15 mg/L|peCHK", Sample_Name, ignore.case = TRUE), 
     #^removes autospice, ccv, ccb, standards
     !is.na(Sample_Name),                                         # Remove NA
     !grepl("^\\d+$", Sample_Name),                               # removes all numbers
     !grepl("^\\d{1,2}/\\d{1,2}/\\d{2,4}", Sample_Name)) %>%      # removes all date/time
...
#Pull out samples 
samples <- df_all_cor %>%  
  filter(!str_detect(Sample_Name, c("Standard|Chk|CHK|Duplicate|Spike|Blank|ammonia|mg/L|CCV|CCB")))     
