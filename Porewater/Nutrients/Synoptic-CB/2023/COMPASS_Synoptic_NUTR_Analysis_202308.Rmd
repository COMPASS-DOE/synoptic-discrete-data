---
title: "Synoptic_CB_Nutrients_202308"
author: "August 2023"
date: "`r Sys.Date()`"
output: 
  pdf_document:
...
#identify the files you want to read in 
  ...
  NOx_files <- c("Raw Data/SEAL_COMPASS_Synoptic_NOx_Aug2023_1.csv",
                 "Raw Data/SEAL_COMPASS_Synoptic_NOx_Aug2023_2.csv",
                 "Raw Data/SEAL_COMPASS_Synoptic_NOx_Aug2023_3.csv")
  NH3_PO4_files <- c("Raw Data/SEAL_COMPASS_Synoptic_NH3_PO4_Aug2023_1.csv", 
                     "Raw Data/SEAL_COMPASS_Synoptic_NH3_PO4_Aug2023_2.csv",
                     "Raw Data/SEAL_COMPASS_Synoptic_NH3_PO4_Aug2023_3.csv")
...
# Define the file path for QAQC log file - NO Need to change just check year 
  ...
  final_path <- "Processed Data/COMPASS_Synoptic_Nutrients_202308.csv"

#record any notes about the run or anything other info here: 
  run_notes <- "Duplicates are out of range because of detection limit, too small of values"
...
if (dil_dups_string != "") {
  ...
  # Check for dilution in duplicates
  if (any(df_duplicates$Dilution > 0, na.rm = TRUE)) {
    message("Dilution Present, Need to Correct")
  } else {
    message("No Dilution, Most likely sample name input error, refer to sample log and next chunk")
  }
...
df_all_cor <- df_all %>%
  ...
  mutate(is_dup = grepl("Duplicate", Sample_Name, ignore.case = TRUE)) %>%
  group_by(Sample_Name, Test) %>%
  filter(
    if (any(Dilution > 1, na.rm = TRUE)) {
      Dilution == max(Dilution, na.rm = TRUE)
    } else {
      TRUE
    }
  ) %>%
...
df_all_cor <- df_all %>%
  ...
  mutate(
    ...
    Conc_dilcorr = Conc * Dilution,
    
    # Assign a unique ID for pairing
    Pair_ID = ifelse(
    ...
    # Preserve original sample name for QA
    Original_Name = Sample_Name
  ) %>%
  group_by(Pair_ID, Test) %>%
  filter(
...
df_PO4_dup <- df_all_cor %>%
  ...
  filter(
    ...
    !is.na(Sample_Name),
    !grepl("^\\d+$", Sample_Name),
    !grepl("^\\d{1,2}/\\d{1,2}/\\d{2,4}", Sample_Name)
  ) %>%
  arrange(row_number()) %>%  # ensure original order is preserved
  mutate(Pair_ID = ifelse(
    grepl("Duplicate", Sample_Name, ignore.case = TRUE),
    lag(Sample_Name),  # assign the Sample_Name of the previous row
    Sample_Name         # assign own name if not a duplicate
  )) %>%
  group_by(Pair_ID) %>%
  filter(n() > 1) %>%  # keep only true pairs
  ungroup() %>%
  transmute(
    Pair_ID,
    Sample_Name,
    Conc,
    Type = ifelse(grepl("Duplicate", Sample_Name, ignore.case = TRUE), "Duplicate", "Original")
  )
...
```{r Rhizon Sample Wrangle, echo = FALSE}
