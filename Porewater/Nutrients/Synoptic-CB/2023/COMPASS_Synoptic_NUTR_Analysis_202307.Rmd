...
cat("Run Information: Please Read
") #lets you know what section you're in 
...
# Define the file path for QAQC log file - NO Need to change just check year 
  ...
  final_path <- "Processed Data/COMPASS_Synoptic_Nutrients_202307.xlsx"

#record any notes about the run or anything other info here: 
  run_notes <- "Run Notes: peChks for NOx and NH3 out of range, but all other
  run metrics looked fine, so we are accepting the run with flags.
  Blanks and Duplicates: Flags for NOx blanks out of range - because samples
  have bdl concentrations, same problem for out of range duplicates"
...
NOx_peChk <- df_all_cor %>%
  ...
  summarise(
    ...
    NOx_pe_flag = ifelse(abs(pct_diff_Chk) < 20, "YES, Pass", "NO, rerun"))
...
NH3_peChk <- df_all_cor %>%
  ...
  summarise(
    ...
    NH3_pe_flag = ifelse(abs(pct_diff_Chk) < 20, "YES, Pass", "NO, rerun"))
...
PO4_peChk <- df_all_cor %>%
  ...
  summarise(
    ...
    PO4_pe_flag = ifelse(abs(pct_diff_Chk) < 20, "YES, Pass", "NO, rerun"))
  


#flag write out 
ifelse(NOx_peChk$pct_diff_Chk >= chk_flag, 
       "NOx pe Check has a % Difference >20% - REASSESS",  
       "NOx pe Check has a % Difference <20% - PROCEED")
cat("Run mean =", NOx_peChk$Mean_Chk_Conc, "\n")
cat("Expected  =", NOx_pe, "\n\n")
ifelse(NH3_peChk$pct_diff_Chk >= chk_flag, 
       "NH3 pe Check has a % Difference >20% - REASSESS",  
       "NH3 pe Check has a % Difference <20% - PROCEED")
cat("Run mean =", NH3_peChk$Mean_Chk_Conc, "\n")
cat("Expected  =", NH3_pe, "\n\n")
ifelse(PO4_peChk$pct_diff_Chk >= chk_flag, 
       "PO4 pe Check has a % Difference >20% - REASSESS",  
       "PO4 pe Check has a % Difference <20% - PROCEED")
