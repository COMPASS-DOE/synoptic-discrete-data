...
#record any notes about the run or anything other info here: 
  run_notes <- "NOx Blanks are out of range because of the slightly negative y intercept of the standard curve and the dups are out of range becasue of their very low concentrations"
...
replacements <- c("list out the samples that we need to replace")
...
df_duplicates <- df_all %>% 
  ...
  filter(!(Sample_Name %in% c("Duplicate",    "DI",                #removing duplicates, autospikes, ccv/bs 
...
# Identify problematic duplicates: duplicated samples that have no diluted version
...
df_all_cor <- df_all %>%
  ...
  mutate(
    ...
    Conc_dilcorr = Conc * Dilution,     #calculate a corrected conc
