...
#record any notes about the run or anything other info here: 
  run_notes <- "This run was stopped and restarted part way through due to
  insufficient water and/or reaction segments. 
  There are fewer spikes for PO4 and NH3 in this run due to stop and rerun.
  NH3 peCheck is high, but other run metrics okay, so accepting this run." 
...
#This run was started and then restarted, so there are duplicated samples 
#this chunk deals with this issue by taking the later run 

# If run_date is a character column, convert to POSIXct
df_all <- df_all %>%
  mutate(Run_Time = as.POSIXct(Run_Time, format = "%m/%d/%Y %H:%M"))

#we will need to take the one with a later date and remove the older ones 
# Keep only the latest run_date per sample_name + run_number
df_latest <- df_all %>%
  group_by(Sample_Name, Run_Number) %>%
  slice_max(Run_Time, n = 1, with_ties = FALSE) %>%
  ungroup()
...
#Pull out samples 
samples <- df_all_cor %>%  
  filter(!str_detect(Sample_Name, c("Standard|Chk|CHK|Duplicate|Spike|Blank|ammonia|mg/L|CCV|CCB|DI")))     
...
df_all_clean <- df_all_clean %>%
  ...
  mutate(Sample_Name = case_when(
      Site == "SWH" & Zone == "SWAMP" ~ gsub("_UP_", "_SWAMP_", Sample_Name), 
      Site == "SWH" & Zone == "UP" ~ gsub("_UPCON_", "_UP_", Sample_Name), 
      TRUE ~ Sample_Name
    )) 
...
write_csv(final_data, final_path)
