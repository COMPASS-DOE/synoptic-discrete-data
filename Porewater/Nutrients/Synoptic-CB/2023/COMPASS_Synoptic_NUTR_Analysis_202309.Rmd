---
title: "Synoptic_CB_Nutrients_2023_Sept"
author: "September"
date: "`r Sys.Date()`"
output: 
  pdf_document:
...
#identify the files you want to read in 
  ...
  NOx_files <- c("Raw Data/SEAL_COMPASS_Synoptic_NOx_Sept2023_1.csv",
                 "Raw Data/SEAL_COMPASS_Synoptic_NOx_Sept2023_2.csv",
                 "Raw Data/SEAL_COMPASS_Synoptic_NOx_Sept2023_3.csv")
  NH3_PO4_files <- c("Raw Data/SEAL_COMPASS_Synoptic_NH3_PO4_Sept2023_1.csv", 
                     "Raw Data/SEAL_COMPASS_Synoptic_NH3_PO4_Sept2023_2.csv",
                     "Raw Data/SEAL_COMPASS_Synoptic_NH3_PO4_Sept2023_3.csv")
...
# Define the file path for QAQC log file - NO Need to change just check year 
  ...
  final_path <- "Processed Data/COMPASS_Synoptic_Nutrients_202309.csv"

#record any notes about the run or anything other info here: 
  run_notes <- "NOx Blanks are out of range because of the slightly negative y intercept of the standard curve and the dups are out of range becasue of their very low concentrations. AUtospikes for PO4 and NH3 are not estimated for this run, flags reported. Tean pe check concentration for PO4 was the concentration of the last run since it was the latest."
...
## TEMPORARY CHUNK FOR SEPT 2023 - Run finished then restarted for dilutions 
```{r TEMPORARY CHUNK, echo = FALSE}

#cat("Replace diluted samples to allow for qaqc - see GitHub issue #32")

#replacements <- c("GCW_202309_WC_LysB_45cm", "GCW_202309_SW_B", "MSM_202309_UP_LysB_10cm", "peChk_NH3_PO4", "GCW_202309_WC_LysB_45cm", "GCW_202309_SW_B", "peChk_NH3_PO4")

#we will need to take the one with a later date and remove the older ones 

# Unsure why in the dilutions read out only MSM_202309_Up_LysB_10cm is coming out when there are more repeats including pe check
...
NOx_peChk <- df_all_cor %>%
  ...
  summarise(
    Mean_Chk_Conc = mean(Conc_dilcorr, na.rm = TRUE),
    SD_Chk_Conc = sd(Conc_dilcorr, na.rm = TRUE),
...
PO4_peChk <- df_all_cor %>%
  filter(Test == "o-PHOS 0.3", 
         str_detect(Sample_Name, "peChk_NH3")) %>%
  group_by(Sample_Name) %>%
  slice_max(order_by = Run_Time, n = 1) %>%
  ungroup() %>%
  summarise(
    Mean_Chk_Conc = mean(Conc_dilcorr, na.rm = TRUE),
    SD_Chk_Conc = sd(Conc_dilcorr, na.rm = TRUE),
...
#making column for flag 
df_all_cor$NOx_flag <- ""
df_all_cor$NH3_flag <- ""
df_all_cor$PO4_flag <- ""

#filling flag into column
...
ifelse(PO4_spk_percent >= chks_flag, 
       "Flag not calculated for this run",
       "Flag not calculated for this run")
