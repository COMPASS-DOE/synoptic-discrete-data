...
log <- bind_rows(stds_log, add_log)
...
## Dilution Corrections - need to ensure the latest dilution is kept
...
df_duplicates <- df_all %>% 
  ...
  filter(!(Sample_Name %in% c("Duplicate",                    #removing duplicates, autospikes, ccv/bs 
                              "CCV", "CCB", "1mg/L ammonia", 
                              "Blank", "Auto Spike", 
                              "0.15 mg/L")) &
  !startsWith(Sample_Name, "Standard_")) %>%                  #removing standards
  ungroup() %>%
...
#pulling them out into a string and  listing them only once
duplicates_string <- df_duplicates %>%
  distinct(Sample_Name) %>%
  pull(Sample_Name) %>%
  paste(collapse = ", ")

#report out if multiple
ifelse(duplicates_string != "", paste("Duplicated samples:", duplicates_string), "No duplicated samples.")

#report if those samples have a dilution 
ifelse(any(df_duplicates$Dilution > 0, na.rm = TRUE),   # condition: any dilution > 0 ignoring NAs
  "Dilution Present, Need to Correct",
  "No Dilution, Most likely sample name input error, refer to sample log")

#If then statement to see if those are diluted 
#ifelse(df_all$Dilution > 0, na.rm = TRUE), ""
   #WAIT ON THIS WHEN CHECKING SMART EX DATA CAUSE WILL PROB BE TAKEN CARE OF IN THE CODE ABOVE

#If then statement to keep the run that has the highest dilution 
df_all_cor <- df_all %>% 
  ...
  group_by(Sample_Name, Test) %>%
  filter(Dilution == ifelse(any(Dilution == 1), 1, max(Dilution, na.rm = TRUE))) %>%
  ungroup()
...
Nox_dup_chk <- df_NOx_dup %>%
  ...
  summarise(
    ...
    NOx_diff_flag = ifelse(CV < 10, "YES", "NO, rerun"),
    .groups = "drop") 
...
NH3_dup_chk <- df_NH3_dup %>%
  ...
  summarise(
    ...
    CV = (SD_Conc / Mean_Conc) * 100,
    pct_diff = abs(Dup_Conc - OG_Conc) / Mean_Conc * 100,
    NH3_diff_flag = ifelse(CV < 10, "YES", "NO, rerun"),
    .groups = "drop") 

PO4_dup_chk <- df_NOx_dup %>%
  group_by(Pair_ID) %>%  # Use Pair_ID as it groups Originals and Duplicates
  filter(n_distinct(Type) == 2) %>%  # Keep only groups that have both Original and Duplicate
  summarise(
    OG_Conc = Conc[Type == "Original"],
    ...
    CV = (SD_Conc / Mean_Conc) * 100,
    pct_diff = abs(Dup_Conc - OG_Conc) / Mean_Conc * 100,
    PO4_diff_flag = ifelse(CV < 10, "YES", "NO, rerun"),
    .groups = "drop") 
...
#dup flag report out
ifelse(NOx_dup_percent >= 60, 
       ">60% of Duplicates have a CV <10% - PROCEED",
       "<60% of Carbon Duplicates have a CB <10% - REASSESS")
ifelse(NH3_dup_percent >= 60, 
       ">60% of Duplicates have a CV <10% - PROCEED",
       "<60% of Carbon Duplicates have a CB <10% - REASSESS")
ifelse(PO4_dup_percent >= 60, 
       ">60% of Duplicates have a CV <10% - PROCEED",
       "<60% of Carbon Duplicates have a CB <10% - REASSESS")
...
NOx_dups_plot <- ggplot(data =Nox_dup_chk, aes(x =Pair_ID, y =CV, fill=NOx_diff_flag)) +
        geom_bar(stat = 'identity') + 
        theme_classic() + 
        labs(x= "Sample ID", y="NOx N (mg/L)", title = "NOx Duplicate CV") + 
...
NH3_dups_plot <- ggplot(data = NH3_dup_chk, aes(x =Pair_ID, y =CV, fill=NH3_diff_flag)) +
        geom_bar(stat = 'identity') + 
        theme_classic() + 
        labs(x= "Sample ID", y="NH3 N (mg/L)", title = "NH3 Duplicate CV") + 
        scale_fill_manual(values = c("YES" = "darkslateblue", "NO, rerun" = "darkred")) +
        theme(legend.position="none") +  
        geom_hline(yintercept=10, linetype="dashed", 
              color = "black", size=1) + 
...
PO4_dups_plot <- ggplot(data =PO4_dup_chk, aes(x =Pair_ID, y =CV, fill=PO4_diff_flag)) +
       geom_bar(stat = 'identity') + 
       theme_classic() + 
        labs(x= "Sample ID", y="PO4 P (mg/L)", title = "PO4 Duplicate CV") + 
       scale_fill_manual(values = c("YES" = "darkslateblue", "NO, rerun" = "darkred")) +
       theme(legend.position="none") +  
       geom_hline(yintercept=10, linetype="dashed", 
                color = "black", size=1) + 
...
#spk flag report out
ifelse(NOx_spk_percent >= 60, 
       ">60% of Spikes have a CV <10% - PROCEED",
       "<60% of Carbon Spikes have a CV <20% - REASSESS")
ifelse(NH3_spk_percent >= 60, 
       ">60% of Spikes have a CV <10% - PROCEED",
       "<60% of Carbon Spikes have a CV <20% - REASSESS")
ifelse(PO4_spk_percent >= 60, 
       ">60% of Spikes have a CV <10% - PROCEED",
       "<60% of Carbon Spikes have a CV <20% - REASSESS")
...
NOx_spk_plot <- ggplot(data =df_spk_NOx_chk, aes(x =Pair_ID, y =CV, fill=Spike_diff_flag)) +
        ...
        scale_fill_manual(values = c("YES" = "aquamarine", "NO, check" = "darkred")) +
        theme(legend.position="none") + 
...
df_matrix <- df_spk_combo%>%
  ...
  group_by(Test)
  ifelse(df_matrix$Spike_diff_flag == "YES", "NO Matrix Effect, PROCEED",
       ">20% CV in seawater auto spikes - REASSESS Matrix Effect")
...
viz_NOx_plot <-  ggplot(data = NOx_forplot, aes(x = Zone, y = Conc, fill=Zone)) +
       geom_bar(stat = 'identity') + 
        ...
        scale_fill_manual(values = c("Upland" = "#20063B", 
                                     "Transition" = "#FFBC42", 
                                     "Swamp" = "darkgrey",
                                     "Wetland" = "#419973", 
                                     "Surface Water" = "#25ABE6")) +
        theme_classic() + 
        labs(x= " ", y="NOx (mg/L)", title="NOx by Site and Zone") + 
        theme(legend.position="none") 
...
viz_NH3_plot <- ggplot(data = NH3_forplot, aes(x = Zone, y = Conc, fill=Zone)) +
       geom_bar(stat = 'identity') + 
        facet_grid(~factor(Site, levels=c('GCReW', 
                                          'MSM', 
                                          'Goodwin Island', 
                                          "SWH"))) + 
        scale_fill_manual(values = c("Upland" = "#20063B", 
                                     ...
                                     "Surface Water" = "#25ABE6")) +
        theme_classic() + 
        labs(x= " ", y="NH3 (mg/L)", title="NH3 by Site and Zone") + 
        theme(legend.position="none") 
...
viz_PO4_plot <- ggplot(data = PO4_forplot, aes(x = Zone, y = Conc, fill=Zone)) +
       geom_bar(stat = 'identity') + 
        ...
        scale_fill_manual(values = c("Upland" = "#20063B", 
                                     ...
                                     "Surface Water" = "#25ABE6")) +
        theme_classic() + 
        labs(x= " ", y="PO4 (mg/L)", title="PO4 by Site and Zone") + 
        theme(legend.position="none") 
...
df_wide <- df %>%
  pivot_wider(
    ...
    names_glue = "{.value}_{test}"
  )


#prep the data 
processed_data <- df_all_clean %>%
  select(Sample_Name, Test, Conc, Zone, Replicate, Depth, Absorbance, Dilution, Unit) %>%
  pivot_wider(
    id_cols = Sample_Name,
    names_from = Test,
    values_from = Conc,
