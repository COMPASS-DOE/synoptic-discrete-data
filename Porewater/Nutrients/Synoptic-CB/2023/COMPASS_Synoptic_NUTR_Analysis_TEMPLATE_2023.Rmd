...
#identify the files you want to read in 
  NOx_file <- "Raw Data/SEAL_COMPASS_Synoptic_NOx_April2023_1.csv"
  NH3_PO4_file <- "Raw Data/SEAL_COMPASS_Synoptic_NH3_PO4_April2023_1.csv"
...
#---------------------NOX---------------------
#Read in Raw Data 
dat_NOx <- readr::read_csv(NOx_file) 
dat_NH3_PO4 <- readr::read_csv(NH3_PO4_file)
# Combining Files
...
#Write out to the user whether or not the r2 is above the cutoff of 0.98
  ifelse(NOx_r2 <= r2_cutoff, "NOx Curve r2 is below cutoff! - REASSESS", "NOx Curve r2 GOOD - PROCEED")
  ifelse(NH3_r2 <= r2_cutoff, "NH3 Curve r2 is below cutoff! - REASSESS", "NH3 Curve r2 GOOD - PROCEED")
  ifelse(PO4_r2 <= r2_cutoff, "PO4 Curve r2 is below cutoff! - REASSESS", "PO4 Curve r2 GOOD - PROCEED")
...
if (file.exists(file_path)) {
  ...
  x <- c("Test", "Intercept", "Slope", "R_squared", "run_date", "user")
  colnames(stds_log) <- x
...
Slopes_chk <- ggplot(log, aes(run_date, Slope, col=Test)) + geom_point(size=4) + 
  geom_line() + theme_bw() + labs(title="Slope Drift Assessment", x="Run Date", y="Slope") +
  scale_color_manual(values=c("darkblue", "darkorchid", "darkgreen"))
Slopes_chk
...
NOx_chks_plot <-  ggplot(data = NOx_chks, aes(x = rep, y = Conc, fill=NOx_diff_flag)) +
       geom_bar(stat = 'identity') + 
        ...
        theme_classic() + labs(x= " ", y="NOx (mg/L)", title="Check Stds: NOx") + 
        theme(legend.position="bottom") +  geom_hline(yintercept=NOx_CCV,
