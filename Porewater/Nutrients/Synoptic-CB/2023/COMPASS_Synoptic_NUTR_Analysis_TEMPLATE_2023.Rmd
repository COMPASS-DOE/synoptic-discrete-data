...
df_combo <- bind_rows(dat_NOx_1, dat_NH3_PO4)
...
Slopes_chk <- ggplot(log, aes(run_date, Slope, col=Test)) + geom_point(size=4) + 
  ...
  scale_color_manual(values=c("cadetblue4", "blueviolet", "darkolivegreen"))
Slopes_chk
...
#Getting dataframe of just samples 
df_NOx_dup <- df_all %>%
  filter(Test == "Vanadium NOx") %>%
  filter(
...
NH3_dup_percent <- (sum(NH3_chks$pct_diff_flag == "YES")/nrow(NH3_chks))*100
PO4_dup_percent <- (sum(PO4_chks$pct_diff_flag == "YES")/nrow(PO4_chks))*100
