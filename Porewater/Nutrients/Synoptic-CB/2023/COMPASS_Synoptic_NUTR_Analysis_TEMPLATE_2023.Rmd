...
df_spk_NOx_chk <- df_NOx_spk %>%
  ...
  rowwise() %>%# keep only rows with both
   mutate(
    ...
    CV = (SD_Conc / Mean_Conc) * 100,
    Spike_diff_flag = ifelse(Pct_Diff < 10, "YES", "NO, check")) %>%
    ungroup()
...
#spk flag report out
ifelse(NOx_spk_percent >= 60, ">60% of Spikes have a CV <10% - PROCEED",
       "<60% of Carbon Spikes have a CV <10% - REASSESS")
ifelse(NH3_spk_percent >= 60, ">60% of Spikes have a CV <10% - PROCEED",
       "<60% of Carbon Spikes have a CV <10% - REASSESS")
ifelse(PO4_spk_percent >= 60, ">60% of Spikes have a CV <10% - PROCEED",
       "<60% of Carbon Spikes have a CV <10% - REASSESS")
