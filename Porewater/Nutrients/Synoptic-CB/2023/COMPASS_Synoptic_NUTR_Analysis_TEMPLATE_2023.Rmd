...
#hHOW DO YOU WANT THE NON LINEAR COEFFS SAVED SINCE THERE IS NO LONGER A SLOPE 
...
df_duplicates <- df_all %>% 
  ...
  filter(!(Sample_Name %in% c("Duplicate", "CCV", "CCB", "1mg/L ammonia", "Blank", "Auto Spike", "0.15 mg/L")) &
  !startsWith(Sample_Name, "Standard_")) %>%
  ungroup() %>%
  group_by(Test, Sample_Name)%>%
  # Count how many times each Sample_Name appears
  filter(n() > 1) %>%
  ungroup() 
...
NOx_dups_plot <- ggplot(data =Nox_dup_chk, aes(x =sample_name, y =npoc_dups_cv, fill=npoc_dups_cv_flag)) +
        geom_bar(stat = 'identity') + 
...
NH3_dups_plot <- ggplot(data =QAdups, aes(x =sample_name, y =tdn_dups_cv, fill=tdn_dups_cv_flag)) +
        geom_bar(stat = 'identity') + 
...
PO4_dups_plot <- ggplot(data =QAdups, aes(x =sample_name, y =tdn_dups_cv, fill=tdn_dups_cv_flag)) +
       geom_bar(stat = 'identity') + 
       theme_classic() + labs(x= "Sample ID", y="TN (mg/L)") + 
       scale_fill_manual(values = c("YES" = "darkslateblue", "NO, rerun" = "darkred")) +
       theme(legend.position="none") +  
...
df_NOx_spike <- df_all %>%
  filter(Test == "Vanadium NOx") %>%
  filter(
     !grepl("CCV|CCB|Standard|Duplicate|Abs_Chk_20ppt|Abs_Chk_10ppt", Sample_Name, ignore.case = TRUE),
...
df_NH3_spike <- df_all %>%
  filter(Test == "Ammonia 2") %>%
  filter(
     !grepl("CCV|CCB|Blank|1mg/L ammonia|Abs_Chk_20ppt|Abs_Chk_10ppt|Standard|Duplicate", Sample_Name, ignore.case = TRUE),
     #^removes dup, ccv, ccb, standards
     !is.na(Sample_Name),                                         # Remove NA
     !grepl("^\\d+$", Sample_Name),                               # removes all numbers
     !grepl("^\\d{1,2}/\\d{1,2}/\\d{2,4}", Sample_Name)) %>%      # removes all date/time
...
#ADD IF ELSE STATEMENT IF SPIKE COLUMN HAS MORE THAN 2 SAMPLES TO REPORT OUT "WRONG NUMBER OF SAMPLES"
ifelse(NOx_spike_chk$pair_id >= n(2), "check check std -> more than 2 runs",
       "<60% of Carbon Duplicates have a CB <10% - REASSESS")



Nox_spike_chk <- df_NOx_spike %>%
  select(pair_id, Conc, Type) %>%
  reshape(idvar = "pair_id", timevar = "Type", direction = "wide")
  pivot_wider(
    names_from = Type,
    values_from = Conc,
    #values_fn = list(Conc = ~mean(.x, na.rm = TRUE))  # handles any unseen duplication
  ) %>%
  mutate(Spike_Higher = Auto_Spike > Original)

  mutate(
    spike_inc = abs(diff(Conc)) / mean(Conc) * 100,
   spike_flag = ifelse(spike_inc < 10, 'YES', 'NO, rerun')) %>%
  ungroup()
