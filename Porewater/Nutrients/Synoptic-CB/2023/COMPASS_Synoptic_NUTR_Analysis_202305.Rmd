...
author: "May 2023 - DO NOT RUN REQUIRES SAMPLE RERUNS"
date: "`r Sys.Date()`"
output: 
  pdf_document:
    toc: true
    number_sections: true
output_dir: "Run Summaries To Review"
---
...
metadata <- raw_metadata %>%
  # mutate(Transect.Location = gsub("UpCON", "UPCON", Transect.Location)) %>% 
  # mutate(
...
nutr_metadata <- metadata %>%
  dplyr::select(NUTR_ID, Year, Month, Day, YearMonth, Site, Zone, Lysimeter, LysID, Depth_cm, 
...
blk_flag_NOx <- samples %>%
  ...
  pull(Q1)   # Now blk_flag_NOx is just a numeric  
...
Nox_dup_chk <- df_NOx_dup %>%
  ...
  filter(Conc != -0.000815) %>% ##Remove extra sample
  summarise(
...
#dup check PO4 see where multiple dups came back 
PO4_dup_counts <- df_PO4_dup %>%
  group_by(Pair_ID, Type) %>%
  tally(name = "n_obs") %>%
  filter(n_obs > 1) %>%
  ungroup()
print(PO4_dup_counts)


PO4_dup_chk <- df_PO4_dup %>%
  group_by(Pair_ID) %>%  # Use Pair_ID as it groups Originals and Duplicates
  filter(!Sample_Name %in% c("0.15 mg/L", "Abs_Chk_10ppt", "Abs_Chk_20ppt", "Blank")) %>%
  filter(n_distinct(Type) == 2) %>%  # Keep only groups that have both Original and Duplicate
  summarise(
    OG_Conc = Conc[Type == "Original"],
    Dup_Conc = Conc[Type == "Duplicate"],
...
#making olumn for flag to go in
df_all_cor$NOx_flag <- ""
df_all_cor$NH3_flag <- ""
df_all_cor$PO4_flag <- ""
...
#Pull out samples 
samples <- df_all_cor %>%  
  filter(!str_detect(Sample_Name, c("Standard|Chk|Duplicate|Spike|Blank|ammonia|mg/L|CCV|CCB")))     
...
## Pulling Out Rhizon Samples
```{r Rhizon Sample Wrangle, include=FALSE}
