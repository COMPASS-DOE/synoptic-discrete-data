...
all_data <- all_data %>%
  mutate(
    Sample_ID = if_else(
      Zone != "SW",
      # Modify the string:
      Sample_ID <- paste(Site, Date, Zone, Replicate, Depth_cm, sep = "_"), 
      Sample_ID <- paste(Site, Date, Zone, Replicate, sep = "_"), 
    )
  )
...
write.csv(Lys_SW_data, "COMPASS_SynopticCB_SO4_Cl_2022.csv")
