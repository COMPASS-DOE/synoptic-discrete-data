...
Sdat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202206_SO4.txt",sep='\t' , header=T, skip=3)
head(Sdat)
...
#Chloride data 
## Read in raw data file from Dionex - copied and saved as a txt
Cldat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202206_Cl.txt",sep='\t' , header=T, skip=3)
head(Cldat)
...
colnames(dups) <- c('Sample_ID', 'SO4_ppm_dup','SO4_Area_dup', 'Cl_ppm_dup','Cl_Area_dup','SO4_mM_dup', "Cl_mM_dup", "salinity_dup")
head(dups)
...
QAdups$SO4_dups_cv <- (df2$sds/df2$mean) * 100
...
## Pull out spikes and check - with dionex output
```{r}
#Show me the data that we have from the calculations 
head(sampledat)
...
# spks <- spks[ ,-c(2:5, 7,8)]
colnames(spks) <- c('Sample_ID', 'SO4_ppm_spk', 'SO4_Area_spk', 'Cl_ppm_spk', 'Cl_Area_spk', 'SO4_mM_spk', "Cl_mM_spk", "salinity_spk")
head(spks)
...
colnames(IDs) <- c("Analysis_No" , "Site", "Zone", "Replicate", "Depth") #, #"Tree")
IDs$Date <- 202206
head(IDs)
...
write.csv(alldat, file="Processed Data/COMPASS_SynopticCB_PW_Processed_Cl_SO4_202206.csv")           #Change file name
