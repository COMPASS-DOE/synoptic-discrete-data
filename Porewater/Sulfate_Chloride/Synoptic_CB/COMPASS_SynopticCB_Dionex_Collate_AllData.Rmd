...
viz_cl_plot_avgs <- ggplot(Dionex_avg_depth, aes(x = YearMonth, y = mean_Cl, color = Zone, group = Zone, shape = as.character(Depth_cm))) +
  geom_point(size = 3) + 
  geom_errorbar(aes(ymin = mean_Cl - sd_Cl, ymax = mean_Cl + sd_Cl), width = 0.2) + 
  geom_line() + 
  facet_grid( ~ Site, scales="free") +
...
viz_so4_plot_avgs <- ggplot(Dionex_avg_depth, aes(x = YearMonth, y = mean_SO4, color = Zone, group = Zone, shape = as.character(Depth_cm))) +
  geom_point(size = 3) + 
  geom_errorbar(aes(ymin = mean_SO4 - sd_SO4, ymax = mean_SO4 + sd_SO4), width = 0.2) + 
  geom_line() + 
  facet_grid( ~ Site, scales="free") +
