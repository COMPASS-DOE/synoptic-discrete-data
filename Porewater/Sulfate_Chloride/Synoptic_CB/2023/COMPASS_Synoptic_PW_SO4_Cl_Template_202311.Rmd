---
title: "COMPASS_SynopticCB_PW_SO4_Cl_202304_Template"
...
#file path and name for raw summary data file 
    raw_file_name_cl = "Raw Data/COMPASS_Synoptic_CB_MonMon_202311_Cl.txt"  #example
    raw_file_name_so4 = "Raw Data/COMPASS_Synoptic_CB_MonMon_202311_SO4.txt"
    
#file path and name of processed data file 
    processed_file_name = "Processed Data/COMPASS_SynopticCB_PW_Processed_Cl_SO4_202311.csv" #example
...
chks_Cl <- chks_raw %>%
  ...
  mutate(
    cv_Cl = (sd_Cl / mean_Cl),
    flag_Cl = ifelse(cv_Cl < chk_flag, "Chloride Check Standard RSD within Range", "Chloride CHECK STANDARD RSD TOO HIGH - REASSESS")
  )
...
chks_S <- chks_raw %>%
  ...
  mutate(
    ...
    flag_SO4 = ifelse(cv_SO4 < chk_flag, "Sulfate Check Standard RSD within Range", "Sulfate CHECK STANDARD RSD TOO HIGH - REASSESS")
  )
...
chks_merged$Cl_diff_flag <-  ifelse(chks_merged$Cl_diff <= chk_conc_flag, 'YES', 'NO, rerun')

chks_merged$SO4_diff <- ((chks_merged$SO4_ppm - chks_merged$SO4_std_conc)/((chks_raw$SO4_ppm + chks_merged$SO4_std_conc)/2)) * 100
chks_merged$SO4_diff_flag <-  ifelse(chks_merged$SO4_diff <= chk_conc_flag, 'YES', 'NO, rerun')
...
##need to undilute them to check this and then we can calculate and flag if they are within the curve 
