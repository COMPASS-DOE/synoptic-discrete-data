...
chks_merged$Cl_diff_flag <-  ifelse(chks_merged$Cl_diff <= chk_flag_std_cl, 'YES', 'NO, rerun')

chks_merged$SO4_diff <- ((chks_merged$SO4_ppm - chks_merged$SO4_std_conc)/((chks_raw$SO4_ppm + chks_merged$SO4_std_conc)/2)) * 100
chks_merged$SO4_diff_flag <-  ifelse(chks_merged$SO4_diff <= chk_flag_std_s, 'YES', 'NO, rerun')
...
```{r Assess Duplicates, include=FALSE}
...
#this is getting confusing here and needs to be dealt with 
  #need to figure out if we need the dilution correction before we do this because doesn't the instrument already do that? 
  #check check check 
###############The issue I see with using the SO4_mM is that the spiked SO4_mM value is calculated using the same Dilution number as the non-spiked SO4_mM since that is entered into the Dionex software. However, the Dilution would not be the same since you are adding an extra volume of spike which would increase the sample dilution.#############################
...
QAspks$SO4_spks_flag <-  ifelse(QAspks$spk_recovery <=120 & QAspks$spk_recovery >=80 , 'Pass', 'Fail')  #fix 
