...
dat_flagged <- all_dat %>%
  ...
  #  ),
    Cl_flag = if_else(
      blk_avg_c > 0.25 * Cl_ppm,
      if_else(
        npoc_flag != "" & !is.na(Cl_flag),
...
dat_flagged_id$SO4_mM <- (dat_flagged_id$SO4_ppm / s_mw)
dat_flagged_id$Cl_mM <- (dat_flagged_id$Cl_ppm / cl_mw)
...
dat_flagged_id$salinity <- ((1.8070 * dat_flagged_id$Cl_ppm) + 0.026) / 1000
