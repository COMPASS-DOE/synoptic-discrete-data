...
Sdat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202306_SO4.txt",sep='\t' , header=T, skip=3)
head(Sdat)
...
#Chloride data 
## Read in raw data file from Dionex - copied and saved as a txt
Cldat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202306_Cl.txt",sep='\t' , header=T, skip=3)
head(Cldat)
...
##Fix Sample ID's Entered Incorrectly
```{r Fix data entered incorrectly, include = FALSE}
##Data entered incorrectly
all_dat$Sample_ID <- as.character(all_dat$Sample_ID)
all_dat <- all_dat %>%
  mutate(across(where(is.character), ~ str_replace_all(., "_202036_", "_202306_")))
...
write.csv(alldat, file="Processed Data/COMPASS_SynopticCB_PW_Processed_Cl_SO4_202306.csv")           #Change file name
