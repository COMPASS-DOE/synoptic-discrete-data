---
title: "COMPASS_SynopticCB_PW_SO4_Cl_202408"
...
###### Run information - PLEASE CHANGE
  Date_Run = "2025-06-16"  #Date that instrument was run
...
```{r Assess Standard Curves, echo=FALSE, warning=FALSE, message=FALSE}
...
#Chloride check for percent of no, checks to see if it  would warrant reruns 
Perc_cl_flag <- dat_flagged %>% 
  group_by(Cl_flag) %>%
  summarise(no_rows = length(Cl_flag)) 
Perc_cl_flag$Total <- length(dat_flagged$Cl_flag)
Perc_cl_flag$Percent <- (Perc_cl_flag$no_rows / Perc_cl_flag$Total)*100
head(Perc_cl_flag)

#report out if the dups are within range
ifelse(Perc_cl_flag$Percent == 100, "All chloride samples are within the range of the standard curve - PROCEED",
       "Some chloride samples are outside the range of the standard curve - REASSESS")


#Sulfate check for percent of no, checks to see if it would warrant reruns 
Perc_so4_flag <- dat_flagged %>% 
  group_by(SO4_flag) %>%
  summarise(no_rows = length(SO4_flag)) 
Perc_so4_flag$Total <- length(dat_flagged$SO4_flag)
Perc_so4_flag$Percent <- (Perc_so4_flag$no_rows / Perc_so4_flag$Total)*100
head(Perc_so4_flag)

ifelse(Perc_so4_flag$Percent == 100, "All chloride samples are within the range of the standard curve - PROCEED",
       "Some chloride samples are outside the range of the standard curve - REASSESS")
