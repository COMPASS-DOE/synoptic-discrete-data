---
title: "Lucie Stetten Incubation Samples: SO4/Cl"
author: "2025 Samples"
date: "`r Sys.Date()`"
output: 
  pdf_document:
...
###### Run information - PLEASE CHANGE
  Date_Run = "2025-11-24"  #Date that instrument was run
...
#file path and name for raw summary data file 
  raw_file_name_cl = "Raw Data/COMPASS_Lucies_Samples_20251124_Cl.txt"
  raw_file_name_so4 = "Raw Data/COMPASS_Lucies_Samples_20251124_SO4.txt"

#file path and name of processed data file 
  processed_file_name = "Processed Data/COMPASS_Lucies_Samples_Processed_Cl_SO4_20251124.csv" 
...
all_dat$Dilution <-  ifelse(str_detect(all_dat$sample_ID, "Lucie"), 5, all_dat$Dilution)
...
QAspks$SampleVol <- 1
QAspks$SampleVol <-  ifelse(QAspks$Dilution == 5, 1500, QAspks$SampleVol)
...
final_data <- dat_flagged %>% 
  filter(!str_detect(sample_ID, "dup")) %>%  
  filter(!str_detect(sample_ID, "spk")) %>% 
  separate(sample_ID, into = c("Name", "Sample_number"), sep = "_")   %>%
  mutate(
    Project = "Lucie Stetten Incubation Samples",   # new column with same value on every row
...
final_data <- final_data %>%
    select(
         ...
         SO4_ppm, SO4_Conc_mM, SO4_Conc_flag, SO4_QAQC_flag, 
         Cl_ppm, Cl_Conc_mM, Cl_Conc_flag, Cl_QAQC_flag, salinity,
