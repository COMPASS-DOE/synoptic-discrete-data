...
Sdat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202309_SO4a.txt",sep='\t' , header=T, skip=3)
head(Sdat)
...
#Chloride data 
## Read in raw data file from Dionex - copied and saved as a txt
Cldat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202309_Cla.txt",sep='\t' , header=T, skip=3)
head(Cldat)
...
sampledat2 <- sampledat2 %>% #GWI_202207_TR_RHZ_TREE1
  mutate(Sample_ID = gsub("_Tree_", "", Sample_ID), 
         Sample_ID = gsub("Collar_", "Collar", Sample_ID))
...
alldat$'NA' <- NULL
alldat$'NA.1' <- NULL
write.csv(alldat, file="Processed Data/COMPASS_SynopticCB_PW_Processed_Cl_SO4_202309a.csv")           #Change file name
