...
Sdat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202308_SO4.txt",sep='\t' , header=T, skip=3)
head(Sdat)
...
#Chloride data 
## Read in raw data file from Dionex - copied and saved as a txt
Cldat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202308_Cl.txt",sep='\t' , header=T, skip=3)
head(Cldat)
...
all_dat <- all_dat %>% 
  mutate(Sample_ID = gsub("78_SWH_202308_WC_LysC_20cm","78_SWH_202308_WC_LysA_20cm", Sample_ID))
...
alldat$'NA.1' <- NULL
alldat <- alldat[,-7]
write.csv(alldat, file="Processed Data/COMPASS_SynopticCB_PW_Processed_Cl_SO4_202308.csv")           #Change file name
