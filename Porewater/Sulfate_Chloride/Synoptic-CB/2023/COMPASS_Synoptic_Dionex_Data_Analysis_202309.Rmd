...
Sdat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202309_SO4.txt",sep='\t' , header=T, skip=3)
head(Sdat)
...
#Chloride data 
## Read in raw data file from Dionex - copied and saved as a txt
Cldat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202309_Cl.txt",sep='\t' , header=T, skip=3)
head(Cldat)
...
#stds <- stds[-c(17),]   #this is if you need to remove one for any reason
head(stds)
...
alldat <- alldat %>% 
  filter(Sample_ID != "50_SWH_202309_UPCON_LysA_20cm") %>% 
  filter(Sample_ID != "51_SWH_202309_UPCON_LysA_45cm") %>% 
  filter(Sample_ID != "61_SWH_202309_UP_LysB_10cm")


write.csv(alldat, file="Processed Data/COMPASS_SynopticCB_PW_Processed_Cl_SO4_202309.csv")           #Change file name
