...
###### Run information - PLEASE CHANGE
  Date_Run = "2026-08-04"  #Date that instrument was run
  Run_by = "Zoe Read"  #Instrument user 
  Script_run_by = "Zoe Read" #Code user 
  run_notes = "1/6 SO4 dups was bad." #any notes from the run
...
#file path and name for raw summary data file 
  raw_file_name_cl = "Raw Data/COMPASS_Synoptic_CB_MonMon_202607_GCW_SWH_Cl.txt"
  raw_file_name_so4 = "Raw Data/COMPASS_Synoptic_CB_MonMon_202607_GCW_SWH_SO4.txt"

#file path and name of processed data file 
  processed_file_name = "Processed Data/COMPASS_SynopticCB_PW_Processed_Cl_SO4_GCW_SWH_202607.csv" 
...
##Fix samples entered incorrectly
```{r fix sample IDs, include = FALSE}

# ##Fix samples that were entered incorrectly
# all_dat <- all_dat1 %>%
#   mutate(sample_ID = ifelse(str_detect(sample_ID, "202605"), paste(sample_ID, "rerun", sep = "_"), sample_ID)) %>%
#   mutate(sample_name = ifelse(str_detect(sample_name, "202605"), paste(sample_name, "rerun", sep = "_"), sample_name))
...
all_dat$Dilution <-  ifelse(str_detect(all_dat$sample_ID, "202607_MSM_TR_LysC_20cm"), 100, all_dat$Dilution)
...
all_dat$Dilution <-  ifelse(str_detect(all_dat$sample_ID, "Lucie"), 50, all_dat$Dilution)
...
all_data_flagged1 <- dat_flagged %>%
  filter(!str_detect(sample_ID, "dup")) %>%
  filter(!str_detect(sample_ID, "Lab Blank")) %>%
  filter(!str_detect(sample_ID, "spk")) %>%
  mutate(sample_ID = ifelse(sample_ID == "202607_MSM_TR_LysC_20cm ", paste0("NA_", sample_ID), sample_ID)) %>% 
  mutate(sample_ID = ifelse(str_detect(sample_ID, "202607"), sub("^[^_]+_", "", sample_ID), sample_ID))
  #select(!Excess_Info)
 
##Reformat Sample IDs
#202607_MSM_UP_LYSA_10CM --> GCW_202605_UP_LYSA_10CM
all_data_flagged <- all_data_flagged1 %>%
  mutate(sample_ID = toupper(sample_ID)) %>% 
  mutate(Site_Code = stringr::str_extract(sample_ID, 'GCW|MSM|SWH|GWI'),
         Date = stringr::str_extract(sample_ID, "[0-9]{6}"),
         Zone = stringr::str_extract(sample_ID, 'UP|SWAMP|TR|WC|_SW_'),
         Zone = gsub("_", "", Zone), 
         LysID = stringr::str_extract(sample_ID, "_A|_B|_C|LYSA|LYSB|LYSC"), 
         LysID = gsub("_", "", LysID), 
         Depth = stringr::str_extract(sample_ID, "10CM|20CM|45CM")) %>% 
  mutate(sample_ID = paste(Site_Code, Date, Zone, LysID, Depth, sep = "_")) %>% 
...
#Remove Lucie's test samples
data_plotting <- data_plotting %>%  
  filter(!str_detect(sample_name, "Lucie")) 
