...
Sdat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202209_SO4.txt",sep='\t' , header=T, skip=3)
head(Sdat)
...
#Chloride data 
## Read in raw data file from Dionex - copied and saved as a txt
Cldat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202209_Cl.txt",sep='\t' , header=T, skip=3)
head(Cldat)
...
##Based on sample list order, 6_202209_MSM_TR_LysC_10cm was probably actually 6_202209_MSM_UP_LysC_10cm
all_dat <- all_dat %>% 
  mutate(Sample_ID = gsub("6_202209_MSM_TR_LysC_10cm","6_202209_MSM_UP_LysC_10cm", Sample_ID))


##Remove samples that were ran several times and had different values
all_dat <- all_dat %>% 
  filter(!str_detect(Sample_ID, "29_202209_GWI_UP_LysA_20cm")) %>% 
  filter(!str_detect(Sample_ID, "30_202209_GWI_UP_LysA_20cm")) 
...
colnames(IDs) <- c("Analysis_No" , "Date","Site", "Zone", "Replicate", "Depth", "Tree")
head(IDs)
...
## Make final dataframe with IDs  
```{r}
write.csv(alldat, file="Processed Data/COMPASS_SynopticCB_PW_Processed_Cl_SO4_202209.csv")  #Change file name
