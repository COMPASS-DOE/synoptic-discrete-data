---
title: "Dionex_COMPASS_November2022"
...
Sdat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202211_SO4.txt",sep='\t' , header=T, skip=3)
head(Sdat)

##Fix Sample IDs that were entered incorrectly
Sdat <- Sdat %>% 
  mutate(X.1 = ifelse(X == "192", paste0(X.1, "_dup"), X.1)) %>% 
  mutate(X.1 = ifelse(X == "163", paste0(X.1, "_spk"), X.1))
...
#Chloride data 
## Read in raw data file from Dionex - copied and saved as a txt
Cldat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202211_Cl.txt",sep='\t' , header=T, skip=3)
head(Cldat)
...
all_dat <- all_dat %>% 
  filter(!str_detect(Sample_ID, "202211_MSM_TR_LysB"))
...
## Make final dataframe with IDs  
```{r}
write.csv(alldat, file="Processed Data/COMPASS_SynopticCB_PW_Processed_Cl_SO4_202211.csv")  #Change file name
