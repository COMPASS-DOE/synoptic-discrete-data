---
title: "Dionex_COMPASS_August2022"
...
Sdat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202208_SO4.txt", sep='\t' , header=T, skip=3)
head(Sdat)
...
#Chloride data 
## Read in raw data file from Dionex - copied and saved as a txt
Cldat <- read.table("Raw Data/COMPASS_Synoptic_CB_MonMon_202208_Cl.txt",sep='\t' , header=T, skip=3)
head(Cldat)
...
##Remove samples that were ran several times and seemed to increase over time for SO4
all_dat <- all_dat %>% 
  filter(Sample_ID != "33_GWI_202208_UP_LysB_10cm")
...
colnames(IDs) <- c("Analysis_No" , "Site", "Date", "Zone", "Replicate", "Depth", "Tree")
head(IDs)
...
write.csv(alldat, file="Processed Data/COMPASS_SynopticCB_PW_Processed_Cl_SO4_202208.csv")    #Change file name
