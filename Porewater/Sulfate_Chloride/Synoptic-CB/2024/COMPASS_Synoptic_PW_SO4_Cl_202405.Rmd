...
###### Run information - PLEASE CHANGE
  Date_Run = "2024-06-04"  #Date that instrument was run
  Run_by = "Zoe Read"  #Instrument user 
  Script_run_by = "Zoe Read" #Code user 
  run_notes = "Samples were put in the wrong vials, so 
  _MSM_202405_UP_LysA_ was switched to _MSM_202405_UP_LysB_, 
  _MSM_202405_UP_LysB_ was switched to _MSM_202405_UP_LysC_, 
  _MSM_202405_UP_LysC_ was switched to _MSM_202405_UP_LysA_, 
  Two SO4 duplicates had high CVs. 
  Some sample IDs are missing from metadata: 
  GWI_202405_UP_LYSC_45CM - this has values of 0 for SO4 and Cl so was likely just Zn Ac and was removed. 
...
##Fix samples that were entered incorrectly
  samples_to_remove <- "GWI_202405_UP_LYSC_45CM"
  Old_ID_1 = "NA"
  New_ID_1 = "NA"

###### File Names - PLEASE CHANGE 
#file path and name for raw summary data file 
  raw_file_name_cl = "Raw Data/COMPASS_Synoptic_CB_MonMon_202405_Cl.txt"
  raw_file_name_so4 = "Raw Data/COMPASS_Synoptic_CB_MonMon_202405_SO4.txt"

#file path and name of processed data file 
  processed_file_name = "Processed Data/COMPASS_SynopticCB_PW_Processed_Cl_SO4_202405.csv" 
...
if (any(grepl("_MSM_202405_UP_LysA_10", all_dat$sample_ID))) {
  all_dat <- all_dat %>%
    mutate(
      sample_ID = case_when(
        grepl("_MSM_202405_UP_LysA_", sample_ID) ~ sub("LysA_", "LysB_", sample_ID),
        grepl("_MSM_202405_UP_LysB_", sample_ID) ~ sub("LysB_", "LysC_", sample_ID),
        grepl("_MSM_202405_UP_LysC_", sample_ID) ~ sub("LysC_", "LysA_", sample_ID),
        TRUE ~ sample_ID
      )
    )
}

if (any(grepl("_MSM_202405_UP_LysA_10", all_dat$sample_name))) {
  all_dat <- all_dat %>%
    mutate(
      sample_name = case_when(
        grepl("_MSM_202405_UP_LysA_", sample_name) ~ sub("LysA_", "LysB_", sample_name),
        grepl("_MSM_202405_UP_LysB_", sample_name) ~ sub("LysB_", "LysC_", sample_name),
        grepl("_MSM_202405_UP_LysC_", sample_name) ~ sub("LysC_", "LysA_", sample_name),
        TRUE ~ sample_ID
      )
    )
}
...
chks_merged$SO4_diff <- (abs(chks_merged$SO4_ppm - chks_merged$SO4_std_conc)/((chks_raw$SO4_ppm + chks_merged$SO4_std_conc)/2)) * 100
...
all_data_flagged$sample_name <- toupper(all_data_flagged$sample_name)

####Fix Sample IDs####
all_data_flagged <- subset(all_data_flagged, !(sample_ID %in% samples_to_remove))
...
final_data$SO4_Conc_mM[is.na(final_data$SO4_Conc_mM)] <- 0
final_data$Cl_Conc_mM[is.na(final_data$Cl_Conc_mM)] <- 0
