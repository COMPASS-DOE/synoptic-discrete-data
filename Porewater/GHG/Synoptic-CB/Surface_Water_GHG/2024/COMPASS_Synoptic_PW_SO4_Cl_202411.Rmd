...
###### Run information - PLEASE CHANGE
  Date_Run = "2025-06-23"  #Date that instrument was run
  Run_by = "Zoe Read"  #Instrument user 
  Script_run_by = "Zoe Read" #Code user 
  run_notes = "Some sample IDs are missing from metadata: GCW_202411_TR_LYSC_45CM,
  MSM_202411_WC_LYSC_10CM; 
  One Cl chk std and several SO4 chk stds are lower than the expected concentration; 
  One SO4 dup had high CV; 
  3 samples had SO4 adl. 
  Another note: These samples These samples came to room temperature during storage 
  due to the refrigerator dying. 
  The data does not appear to be impacted, but is noted here." #any notes from the run
...
#file path and name for raw summary data file 
  raw_file_name_cl = "Raw Data/COMPASS_Synoptic_CB_MonMon_202411_Cl.txt"
  raw_file_name_so4 = "Raw Data/COMPASS_Synoptic_CB_MonMon_202411_SO4.txt"

#file path and name of processed data file 
  processed_file_name = "Processed Data/COMPASS_SynopticCB_PW_Processed_Cl_SO4_202411.csv" 
...
########Change Sample_ID Because it was entered wrong###############
## Change MSM_202411_WC_LysC_10cm to MSM_202411_WC_LysB_10cm 
all_dat$sample_name[all_dat$sample_name == "103_MSM_202411_WC_LysC_10cm"] <- "103_MSM_202411_WC_LysB_10cm"
all_dat$sample_ID[all_dat$sample_ID == "103_MSM_202411_WC_LysC_10cm"] <- "103_MSM_202411_WC_LysB_10cm"
...
if (cl_dups_diff_percent <= chks_flag) {
    all_dat$Cl_QAQC_flag <- ifelse(
    all_dat$Cl_QAQC_flag != "",
    paste0(all_dat$Cl_QAQC_flag, "; Cl dups out of range"),
...
if (so4_dups_diff_percent <= chks_flag) {  
    all_dat$SO4_QAQC_flag <- ifelse(
    all_dat$SO4_QAQC_flag != "",
    paste0(all_dat$SO4_QAQC_flag, "; SO4 dups out of range"),
