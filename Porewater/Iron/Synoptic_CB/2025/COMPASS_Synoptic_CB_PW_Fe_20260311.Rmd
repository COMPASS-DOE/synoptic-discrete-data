---
title: "FeCode_updated2025_20260311"
output: pdf_document
date: "`r Sys.Date()`"
---
...
Date_Run = "20260311"
plates<- c("Plate15", "Plate16", "Plate17")
Std_plates<- c("STD")
...
Experiment="2025"
Run_notes="
The Fe2 diff std curve R squared is 0.94, which is below the 0.985 cutoff but I think is reasonable, 
and Melanie mentioned that this R sqaured might be lower since it is the difference between two curves and is more sensitive. 
The FE_2_high std curve R squared is 0.984, which is close to the 0.985 cutoff
so I think it is reasonable. 
The FE_2_diff slope is slightly lower than previous slopes. 
The check stds for FE3 60 uM and Fe3 9 uM were all bad.
One dup was off for Fe2 and one dup was off for Fe3. 
One spike was too high and one spike looks like the spk solution didn't get added. "#any notes from run
...
dat1_HCV_fixed_FE3 <- dat_HCV_fixed_FE3 %>%
  ...
  summarise(IDs=first(IDs),FE_2_mean = mean(na.omit(FE_2_Conc_Final)), FE_2_sd = sd(na.omit(FE_2_Conc_Final)), FE_2_cv = cv(na.omit(FE_2_Conc_Final)),
            ...
            FE_tot_mean =(mean(FE_2_Conc_Final,na.rm=TRUE)+ mean(FE_3_Conc_Final,na.rm=TRUE)), FE_tot_sd = sqrt(sd(na.omit(FE_3_Conc_Final))^2+sd(na.omit(FE_2_Conc_Final))),
            FE_2_flag = ifelse(mean(na.omit(Abs1)) > mean(FE_2_500uM$Abs1), "High",ifelse(mean(na.omit(Abs1)) < (mean(FE_2_0uM$Abs1)+ MDL), "Below MDL" ,"Within range")),
            FE_3_flag = ifelse(mean(na.omit(Abs2)) > mean(FE_2_100uM$Abs2)|(mean(na.omit(Abs2))-mean(na.omit(Abs1))) > mean(FE_3_100uM$Abs2), "High", 
...
dupsbar_Fe2 <- ggplot(data = QAdups, aes(x = ID_full, y = FE_2_dups_chk, fill=FE_2_dups_flag)) +
       geom_bar(stat = 'identity') + 
        scale_fill_manual(values=c("OK" ='darkgreen' , "Rerun" = 'darkred')) + 
        theme_classic() + labs(x= "Sample ID", y="Difference Between Fe 2 Duplicates (%)") + 
...
dupsbar_Fe3 <- ggplot(data = QAdups, aes(x = ID_full, y = FE_3_dups_chk, fill=FE_3_dups_flag)) +
       geom_bar(stat = 'identity') + 
       scale_fill_manual(values=c("OK" ='darkgreen' , "Rerun" = 'darkred')) + 
        theme_classic() + labs(x= "Sample ID", y="Difference Between Fe 3 Duplicates (%)") + 
        theme(legend.position="none") +  geom_hline(yintercept=dups_perc_diff, linetype="dashed", 
                color = "black", linewidth=1)
...
dupsbar_Fetot <- ggplot(data = QAdups, aes(x = ID_full, y = FE_tot_dups_chk, fill=FE_tot_dups_flag)) +
       geom_bar(stat = 'identity') + 
       scale_fill_manual(values=c("OK" ='darkgreen' , "Rerun" = 'darkred')) + 
        theme_classic() + labs(x= "Sample ID", y="Difference Between Fe Tot Duplicates (%)") + 
        theme(legend.position="none") +  geom_hline(yintercept=dups_perc_diff, linetype="dashed", 
                color = "black", linewidth=1)
