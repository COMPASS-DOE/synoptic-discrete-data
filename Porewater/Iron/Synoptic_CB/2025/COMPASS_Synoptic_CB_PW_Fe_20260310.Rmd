---
title: "FeCode_updated2025_20260310"
output: pdf_document
date: "`r Sys.Date()`"
---
...
Date_Run = "20260310"
plates<- c("Plate7", "Plate8", "Plate9", "Plate10", "Plate11", "Plate12", "Plate13", "Plate14")
Std_plates<- c("STD")
Run_by = "Melanie Giessner and Zoe Read"#Instrument user
Script_run_by ="Zoe Read"#Code user
Project = "COMPASS_Synoptic_CB"
Experiment="2025"
Run_notes="The Fe2 diff std curve R squared is 0.9747518, which is  below the 0.985 cutoff, 
but Melanie suggested this is okay because this is the difference curve whihc is more sensitive. 
Removed 45 uM Fe_2 B standard because it was off. 
The check stds for FE3 60 uM were all bad, and other check standards were bad on some plates.
<60% of Spikes are within range. We tend to have issues with the spikes. "#any notes from run

#Stds that should be excluded
stds_to_remove<-c("45 uM Fe_2 B")
```
## File paths 
...
summarizeddata_path=paste0("Processed Data/",Project,"_FE_",Date_Run,"_Summary_Data.csv")
fulldata_path=paste0("Processed Data/",Project,"_FE_",Date_Run,"_Data.csv")
shortdata_path=paste0("Processed Data/",Project,"_FE_",Date_Run,"_short_Data.csv")
out_of_Range=paste0("Processed Data/",Project,"_FE_",Date_Run,"_out_of_range_Data.csv")
high_CV=paste0("Processed Data/",Project,"_FE_",Date_Run,"_high_CV_Data.csv")
#QAQC log path
...
dat_aver <- dat1 %>%
  group_by(ID_full) %>%
  summarise(IDs=first(IDs),
            FE_2_mean = mean(na.omit(FE_2_Conc_Final)), 
            FE_2_sd = sd(na.omit(FE_2_Conc_Final)), 
            FE_2_cv = cv(na.omit(FE_2_Conc_Final)),
            FE_3_mean = mean(na.omit(FE_3_Conc_Final)), 
            FE_3_sd = sd(na.omit(FE_3_Conc_Final)), 
            FE_3_cv = cv(na.omit(FE_3_Conc_Final)),
            FE_tot_mean =(mean(FE_2_Conc_Final,na.rm=TRUE)+ mean(FE_3_Conc_Final,na.rm=TRUE)), 
            FE_tot_sd = sqrt(sd(na.omit(FE_3_Conc_Final))^2+sd(na.omit(FE_2_Conc_Final))),
            FE_2_flag = ifelse(mean(na.omit(Abs1)) > mean(FE_2_500uM$Abs1), "High",
                               ifelse(mean(na.omit(Abs1)) < (mean(FE_2_0uM$Abs1)+ MDL), "Below MDL" ,"Within range")),
            FE_3_flag = ifelse(mean(na.omit(Abs2)) > mean(FE_2_100uM$Abs2)|
                              (mean(na.omit(Abs2))-mean(na.omit(Abs1))) > (mean(FE_3_100uM$Abs2)- mean(FE_3_100uM$Abs1)), "High",
                              ifelse(mean(na.omit(Abs2_diff)) < MDL|mean(na.omit(Abs2)) < (mean(FE_2_0uM$Abs2)+ MDL), "Below MDL" ,"Within range")),
            Dilution = first(Dilution),
            Plate = first(Plate),
            QAQC_flag = first(QAQC_flag), 
            Abs1_mean = mean(na.omit(Abs1)), 
            Abs2_mean = mean(na.omit(Abs2)))
...
ifelse(spks_percent >= 60  , ">60% of Spikes are within range",
       "<60% of Spikes are within range - REASSESS")
```
